...
namespace
{
    ...
    bool DcDisabledNotice(ChatHandler* handler)
    {
        ...
        handler->SendSysMessage(
            "이 서버에서 던전 자동 진행이 꺼져 있습니다. mod_dungeon_clear.conf의 DungeonClear.Enable을 1로 설정하고 월드서버를 재시작하세요.");
        return true;
    }
    ...
    bool RunDcCommand(ChatHandler* handler, std::string const& action, std::string const& param = "")
    {
        ...
        if (!issuer)
        {
            handler->SendSysMessage("게임 안에서 사용해야 하는 명령어입니다.");
            return true;
        }

        if (!DungeonClearDispatch::DispatchToTankBots(issuer, action, param))
            handler->SendSysMessage("파티에 탱커 봇이 없습니다.");
    ...
    bool HandleConfig(ChatHandler* handler)
    {
        ...
        handler->SendSysMessage(
            "던전 자동 진행의 현재 설정 (* = 애드온 지정값, H = 영웅 기본값):");
    ...
    bool HandleSpectate(ChatHandler* handler, Optional<std::string> param)
    {
            ...
            if (!name.empty())
            {
                ...
                if (!target)
                {
                    handler->PSendSysMessage(
                        "관전할 수 있는 '{}' 봇이 없습니다. .dc spectate list로 확인하세요.", name);
                    return true;
                }
        ...
        {
            handler->SendSysMessage(
                "사용법: .dc spectate [follow [name] | next | prev | list]");
            return true;
        }
...
public:
    ...
    static bool HandleTestStart(ChatHandler* handler, Tail args)
    {
        ...
        while (in >> word)
        {
            ...
            else if (word.rfind("ilvl=", 0) == 0)
            {
                ...
                if (!ok)
                {
                    handler->SendSysMessage("ilvl은 1~400이며, 제한을 없애려면 none을 사용하세요.");
                    return true;
                }
            }
            else if (word.rfind("quality=", 0) == 0)
            {
                ...
                {
                    handler->SendSysMessage(
                        "quality는 normal|uncommon|rare|epic|legendary 또는 1~5입니다.");
                    return true;
                }
            }
        ...
        if (!party.empty())
        {
            ...
            {
                handler->SendSysMessage(
                    "party= 실행에는 level=, seed=, size=, ilvl=/quality=가 적용되지 않습니다. 캐릭터의 현재 레벨과 장비를 유지하며 명단 인원이 파티 크기가 됩니다.");
                return true;
            }
        ...
        else
        {
            ...
            {
                handler->SendSysMessage("size는 2~40이어야 합니다 (기본 구성 10/25).");
                return true;
            }
    ...
    static bool HandleQueueFillCancel(ChatHandler* handler, Tail playerName)
    {
        ...
        if (name.empty())
        {
            handler->SendSysMessage("사용법: .dc dungeonqueuefill cancel <player>");
            return true;
        }
    ...
    static bool HandleQueueFillTest(ChatHandler* handler, Tail playerName)
    {
        ...
        if (name.empty())
        {
            handler->SendSysMessage("사용법: .dc dungeonqueuefill test <player>");
            return true;
        }
        ...
        if (!target)
        {
            handler->PSendSysMessage("'%s' 플레이어가 접속해 있지 않습니다.", name.c_str());
            return true;
        }
    ...
    static bool HandleTestStop(ChatHandler* handler, Tail selector)
    {
        ...
        if (std::string(selector) == "all" && DcTestPlanManager::Instance().HasActivePlans())
        {
            ...
            handler->SendSysMessage("모든 시험 계획을 중지합니다.");
        }
        std::string msg;
    ...
    static bool HandleTestWatch(ChatHandler* handler, Tail selectorArg)
    {
        ...
        if (selector == "off" || selector == "stop")
        {
            ...
            DcSpectator::Stop(gm);
            if (!wasActive)
                handler->SendSysMessage("실행 중인 관전 카메라가 없습니다. 대기 중인 관전 요청도 취소했습니다.");
            ...
            if (!held.empty())
            {
                ...
                if (insideAWatchedCopy && !gm->IsBeingTeleported())
                {
                    ...
                    handler->SendSysMessage("관전하기 전 위치로 돌아왔습니다.");
                }
        ...
        {
            handler->SendSysMessage("해당 팀의 탱커가 아직 접속하지 않았습니다. 잠시 후 다시 시도하세요.");
            return true;
        }

        handler->PSendSysMessage("{} 관전 중", msg);
        ...
        if (!gm->IsGameMaster())
        {
            ...
            if (!gmModeApplied)
                handler->SendSysMessage("팀에 보이지 않도록 GM 모드를 켰습니다. 관전을 끝내면 원래 상태로 복원됩니다.");
            gmModeApplied = true;
        }
                            ...
                            plan.forceNewInstance))
        {
            ...
            handler->SendSysMessage("해당 인스턴스로 이동할 수 없습니다.");
            return true;
        }
    ...
    static bool HandleTestPlanStart(ChatHandler* handler, Tail args)
    {
        ...
        if (!issuer)
        {
            ...
            {
                handler->SendSysMessage("시험 계획을 시작하지 못했습니다: " + why);
                return true;
            }
    ...
    static bool HandleTestList(ChatHandler* handler)
    {
        ...
        handler->SendSysMessage("지원되는 시험 던전 (.dc test start <token> [heroic]):");
    ...
    static bool HandleTestGear(ChatHandler* handler, Tail args)
    {
        ...
        if (!row)
        {
            handler->SendSysMessage("사용법: .dc test gear <dungeon> [heroic] — .dc test list 참고");
            return true;
        }
        ...
        handler->SendSysMessage("  ilvl=none  아이템 레벨 제한 없음");
        return true;
    }
};
