...
std::string PlayerbotTextMgr::GetBotText(std::string name, std::map<std::string, std::string> placeholders)
{
    ...
    std::string rendered;
    // Missing context is a template contract error, not text to show to players.
    PlayerbotTextTokens::Render(botText, placeholders, rendered);
    return rendered;
}
    ...
    std::map<std::string, std::string> placeholders)
{
    ...
    {
        std::string rendered;
        PlayerbotTextTokens::Render(defaultText, placeholders, rendered);
        return rendered;
    }
...
{
    // A Korean realm must not switch bot templates to English while login counters settle.
