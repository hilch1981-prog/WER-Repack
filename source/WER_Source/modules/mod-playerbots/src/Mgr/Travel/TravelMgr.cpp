...
namespace
{
    std::map<uint32, std::pair<uint32, uint32>> fieldLevelBands;
}

bool WlBotLevelFitsZone(uint32 level, uint32 zoneId)
{
    auto const it = fieldLevelBands.find(zoneId);
    if (it == fieldLevelBands.end())
        return true;
    // Starter regions also serve levels 1-4, although the travel cache starts at 5.
    uint32 const low = it->second.first <= 5 ? 1 : it->second.first;
    return level >= low && level <= it->second.second;
}
...
void TravelMgr::PrepareZone2LevelBracket()
{
    ...
    fieldLevelBands.clear();
    for (auto const& [zoneId, bracket] : zone2LevelBracket)
        fieldLevelBands[zoneId] = {bracket.low, bracket.high};
}
