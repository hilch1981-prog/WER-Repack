...
public:
    ...
    void OnPlayerLogin(Player* player) override
    {
        if (!player->GetSession()->IsBot())
        {
            ...
            if (sPlayerbotAIConfig.enabled || sPlayerbotAIConfig.randomBotAutologin)
            {
                ...
                ChatHandler(player->GetSession()).SendSysMessage(
                    "|cff00ff00플레이어봇:|r 서버의 봇 목표 수는 " + maxAllowedBotCount + "명입니다.");
            }
        }
    }
    ...
    using PlayerScript::OnPlayerCanUseChat;  // keep base overloads visible

    bool OnPlayerCanUseChat(Player* player, uint32 type, uint32 lang, std::string& msg, Player* receiver) override
    {
        // WL: hidden addon-channel traffic must never reach a bot's command
        // pipeline. An addon whisper (GearScore, DBM, Questie...) arrives as an
        // ORDINARY type - CHAT_MSG_WHISPER/PARTY/RAID/GUILD - with lang ==
        // LANG_ADDON; CHAT_MSG_ADDON (0xFFFFFFFF) is never an inbound type, so
        // `lang` is the only usable predicate. Without this the message reaches
        // PlayerbotSecurity::CheckLevelFor with silent = (type != WHISPER) =
        // false, and the bot WHISPERS a refusal at a message the player never
        // saw - the live "bots answer my gearscore addon" bug. Returning true
        // still lets the addon message itself through to its real recipient.
        if (lang == LANG_ADDON)
            return true;
    ...
    {
        if (lang == LANG_ADDON)   // see the whisper hook
            return true;
    ...
    bool OnPlayerCanUseChat(Player* player, uint32 type, uint32 lang, std::string& msg, Guild* /*guild*/) override
    {
        ...
        for (PlayerBotMap::const_iterator it = playerbotMgr->GetPlayerBotsBegin(); it != playerbotMgr->GetPlayerBotsEnd(); ++it)
        {
            ...
            // WL: guard the AI lookup. Every sibling hook null-checks it; this one
            // dereferenced it raw, so a guild member that is a bot without an AI
            // (mid-teardown, or an addclass bot whose AI has been removed) crashed
            // the worldserver on any guild-chat line.
            if (PlayerbotAI* const botAI = PlayerbotsMgr::instance().GetPlayerbotAI(bot))
                botAI->HandleCommand(type, msg, player);
        }
