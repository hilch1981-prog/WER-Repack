...
public:
    ...
    /**
     * @brief Is the calling thread the world thread?
     *
     * The world thread is identified as the thread that runs Update() (the
     * WorldScript::OnUpdate hook). Before the first Update() this returns
     * false for every thread, so early callers simply take the deferred path.
     */
    bool IsWorldThread() const
    {
        return std::this_thread::get_id() == m_worldThreadId.load(std::memory_order_relaxed);
    }
...
private:
    ...
    // Identity of the thread that runs Update() — see IsWorldThread().
    std::atomic<std::thread::id> m_worldThreadId{};
};
