...
{
    // Learn/refresh which thread is the world thread (the OnUpdate hook caller) —
    // IsWorldThread() lets callers decide between inline and deferred execution.
    m_worldThreadId.store(std::this_thread::get_id(), std::memory_order_relaxed);
