...
// Full bot logout, deferred from a map thread. LogoutPlayerBot deletes the
// bot's Player + PlayerbotAI + WorldSession; doing that on a map worker thread
// frees objects other map threads are still ticking (the player itself, its
// auras, its pets) — the core mandates logout only in World::UpdateSessions.
// PlayerbotHolder::LogoutPlayerBot queues this when called off the world
// thread; Execute() re-resolves the bot and runs the logout inline (we ARE on
// the world thread here, so the holder takes its immediate path).
class BotDeferredLogoutOperation : public PlayerbotOperation
{
public:
    BotDeferredLogoutOperation(ObjectGuid botGuid) : m_botGuid(botGuid) {}
    ...
    bool Execute() override
    {
        ...
        // Same holder choice as the AI's own logout path: master-owned bots
        // belong to the master's mgr (its playerBots map), the rest to the
        // random mgr. If the first holder doesn't know the bot, fall through.
        if (Player* master = botAI->GetMaster())
            if (PlayerbotMgr* masterMgr = GET_PLAYERBOT_MGR(master))
                masterMgr->LogoutPlayerBot(m_botGuid);
    ...
    ObjectGuid GetBotGuid() const override { return m_botGuid; }
    uint32 GetPriority() const override { return 100; }   // crash-prevention class
