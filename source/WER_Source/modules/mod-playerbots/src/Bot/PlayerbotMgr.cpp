...
{
    // Logout deletes the bot's Player + PlayerbotAI + WorldSession. From a map
    // worker thread (bot AI self-logout paths) that frees objects other map
    // threads are still ticking — the core mandates logout only happens in
    // World::UpdateSessions (world thread). Defer when called off-thread; the
    // queued operation re-enters here ON the world thread and runs inline.
    if (!PlayerbotWorldThreadProcessor::instance().IsWorldThread())
    {
...
void PlayerbotHolder::OnBotLogin(Player* const bot)
{
    ...
    uint32 const companionCap = std::clamp<uint32>(sConfigMgr->GetOption<uint32>(
        "WowLegends.Companion.MaxLevel", sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)),
        1, sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL));
    uint32 const companionLevel = master ? std::min<uint32>(master->GetLevel(), companionCap) : companionCap;
    if (addClassBot && master && (bot->GetLevel() > companionCap ||
        std::abs(int(companionLevel) - int(bot->GetLevel())) > 3))
    {
        ...
        if (mixedGearScore == 0)
            mixedGearScore = 1;
        PlayerbotFactory factory(bot, companionLevel, ITEM_QUALITY_LEGENDARY, mixedGearScore);
        factory.Randomize(false);
    }
...
bool PlayerbotMgr::HandlePlayerbotMgrCommand(ChatHandler* handler, char const* args)
{
    ...
    {
        handler->PSendSysMessage("|cffff0000플레이어봇 기능이 꺼져 있습니다!");
        return false;
    }
    ...
    if (!m_session)
    {
        handler->PSendSysMessage("접속 중인 캐릭터로만 봇을 추가할 수 있습니다.");
        return false;
    }
    ...
    if (!mgr)
    {
        handler->PSendSysMessage("아직 봇을 조작할 수 없습니다.");
        return false;
    }
...
std::vector<std::string> PlayerbotHolder::HandlePlayerbotCommand(char const* args, Player* master)
{
    ...
    for (std::vector<std::string>::iterator i = chars.begin(); i != chars.end(); i++)
    {
        ...
        // WL: character names are stored capitalized ("Debugalt"); a raw
        // lowercase lookup fails on a real character. Normalize a copy for
        // every CHARACTER-name lookup (account names keep the raw string).
        std::string charLookup = s;
        bool const validCharName = normalizePlayerName(charLookup);
        ...
        if (!strcmp(cmd, "addaccount"))
        {
            ...
            if (!accountId)
            {
                ...
                ObjectGuid charGuid = validCharName
                    ? sCharacterCache->GetCharacterGuidByName(charLookup)
                    : ObjectGuid::Empty;
                if (!charGuid)
                {
...
void PlayerbotMgr::HandleSetSecurityKeyCommand(Player* player, std::string const& key)
{
    ...
    ChatHandler(player->GetSession()).PSendSysMessage("계정 연결용 보안 키를 설정했습니다.");
}
...
void PlayerbotMgr::HandleLinkAccountCommand(Player* player, std::string const& accountName, std::string const& key)
{
    ...
    if (!result)
    {
        ChatHandler(player->GetSession()).PSendSysMessage("계정을 찾을 수 없습니다.");
        return;
    }
    ...
    if (!result)
    {
        ChatHandler(player->GetSession()).PSendSysMessage("보안 키가 올바르지 않습니다.");
        return;
    }
    ...
    ChatHandler(player->GetSession()).PSendSysMessage("계정을 연결했습니다.");
}
...
void PlayerbotMgr::HandleViewLinkedAccountsCommand(Player* player)
{
    ...
    if (!result)
    {
        ChatHandler(player->GetSession()).PSendSysMessage("연결된 계정이 없습니다.");
        return;
    }

    ChatHandler(player->GetSession()).PSendSysMessage("연결된 계정:");
    do
    {
        Field* fields = result->Fetch();
        uint32 linkedAccountId = fields[0].Get<uint32>();
        ...
        {
            ChatHandler(player->GetSession()).PSendSysMessage("- 알 수 없는 계정");
        }
...
void PlayerbotMgr::HandleUnlinkAccountCommand(Player* player, std::string const& accountName)
{
    ...
    ChatHandler(player->GetSession()).PSendSysMessage("계정 연결을 해제했습니다.");
}
