...
public:
    ...
    // WOW LEGENDS: forcedTab >= 0 pins the talent tree (captured by the caller
    // BEFORE any resetTalents) instead of dice-rolling RandomClassSpecProb -
    // how a held bot keeps the spec its owner gave it across re-inits.
    ...
    void InitEquipment(bool incremental, bool second_chance = false, uint32 slotMask = 0xFFFFFFFFu);
    void RepairProfessionEquipment();
    void InitPet();
    void InitAmmo();
