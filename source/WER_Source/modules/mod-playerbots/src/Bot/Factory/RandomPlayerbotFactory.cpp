...
namespace
{
std::string GetPlayerbotNameLocale()
{
    LocaleConstant locale = sWorld->GetDefaultDbcLocale();
    return locale < TOTAL_LOCALES ? localeNames[locale] : localeNames[LOCALE_enUS];
}
}
...
std::string const RandomPlayerbotFactory::CreateRandomGuildName()
{
    ...
    result = CharacterDatabase.Query(
        "SELECT COALESCE(NULLIF(l.name, ''), n.name) FROM playerbots_guild_names n "
        "LEFT JOIN playerbots_guild_names_locale l ON l.name_id = n.name_id AND l.locale = '{}' "
        "LEFT OUTER JOIN guild e ON e.name = COALESCE(NULLIF(l.name, ''), n.name) "
        "WHERE e.guildid IS NULL AND n.name_id >= {} LIMIT 1",
        GetPlayerbotNameLocale(), id);
    if (!result)
    {
        LOG_ERROR("playerbots", "No more names left for random guilds");
        return guildName;
...
void RandomPlayerbotFactory::LoadArenaTeamData()
{
    ...
    QueryResult result = CharacterDatabase.Query(
        "SELECT COALESCE(NULLIF(l.name, ''), n.name) FROM playerbots_arena_team_names n "
        "LEFT JOIN playerbots_arena_team_names_locale l ON l.name_id = n.name_id AND l.locale = '{}' "
        "LEFT OUTER JOIN arena_team e ON e.name = COALESCE(NULLIF(l.name, ''), n.name) "
        "WHERE e.arenateamid IS NULL",
        GetPlayerbotNameLocale());
