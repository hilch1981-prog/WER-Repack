...
void PlayerbotFactory::Randomize(bool incremental)
{
    ...
    // WOW LEGENDS: a HELD bot keeps the spec its owner gave it. Re-init used
    // to dice-roll a fresh talent tree (RandomClassSpecProb), so gearing your
    // hand-specced Holy paladin could hand it back Protection - StretchtGorilla,
    // #bots-and-ai 2026-08-08. The spec MUST be captured HERE: resetTalents
    // below wipes the trees, and InitTalentsTree's own increment branch reads
    // them AFTER the wipe, which is exactly why it never preserved anything.
    // Random bots are exempt on purpose - their periodic re-roll is what keeps
    // the realm's spec mix alive.
    int32 wlKeepTab = -1;
    if (!sRandomPlayerbotMgr.IsRandomBot(bot))
    {
        std::map<uint8, uint32> tabs = AiFactory::GetPlayerSpecTabs(bot);
        if (tabs[0] + tabs[1] + tabs[2] > 0)
            wlKeepTab = int32(AiFactory::GetPlayerSpecTab(bot));
    }
    ...
    {
        uint32 specIndex = InitTalentsTree(false, true, false, wlKeepTab);
                                         ...
                                         int32 forcedTab /*-1*/)
{
    ...
    // WOW LEGENDS: a caller that captured the bot's spec BEFORE wiping talents
    // (Randomize) forces it through here - the increment branch below can't
    // help, because by now the trees it reads are already empty. Known edge:
    // a bear-feral druid refills via the cat template (the form aura the
    // cat/bear split keys on died with the reset). Same tree, so the ROLE
    // survives, which is the promise being kept.
    if (forcedTab >= 0)
    {
        specTab = uint32(forcedTab);
    }
...
{
    // Never select profession-restricted gear for a bot that cannot use it.
    if (!proto || (proto->RequiredSkill &&
        (!bot->HasSkill(proto->RequiredSkill) || bot->GetSkillValue(proto->RequiredSkill) < proto->RequiredSkillRank)) ||
        (proto->RequiredSpell && !bot->HasSpell(proto->RequiredSpell)))
        return false;
...
void PlayerbotFactory::InitEquipment(bool incremental, bool second_chance, uint32 slotMask)
{
    if (!slotMask)
        return;
    if (level < 5)
    {
        if (slotMask != 0xFFFFFFFFu)
            return; // Recovery must never grant unrelated starting-outfit slots.
        // original items
    ...
    for (int32 slot : initSlotsOrder)
    {
        ...
        if (slotMask == 0xFFFFFFFFu && level < 50 && (slot == EQUIPMENT_SLOT_TRINKET1 || slot == EQUIPMENT_SLOT_TRINKET2))
            continue;

        if (slotMask == 0xFFFFFFFFu && level < 30 && (slot == EQUIPMENT_SLOT_NECK || slot == EQUIPMENT_SLOT_HEAD))
            continue;

        if (slotMask == 0xFFFFFFFFu && level < 20 && (slot == EQUIPMENT_SLOT_FINGER1 || slot == EQUIPMENT_SLOT_FINGER2))
            continue;
...
void PlayerbotFactory::InitTradeSkills()
{
    ...
    RepairProfessionEquipment();
}

void PlayerbotFactory::RepairProfessionEquipment()
{
    // Random bots only. Do not reset skills, talents, quests, pets or valid equipment.
    if (!sRandomPlayerbotMgr.IsRandomBot(bot) || bot->IsInCombat())
        return;
    ...
    for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
    {
        ...
        if ((!proto->RequiredSkill || (bot->HasSkill(proto->RequiredSkill) &&
            bot->GetSkillValue(proto->RequiredSkill) >= proto->RequiredSkillRank)) &&
            (!proto->RequiredSpell || bot->HasSpell(proto->RequiredSpell)))
            continue;

        // Profession reassignment must not leave illegal equipment persisted.
        bot->MoveItemFromInventory(INVENTORY_SLOT_BAG_0, slot, true);
        uint32 const itemGuid = item->GetGUID().GetCounter();
        item->DeleteFromInventoryDB(trans);
        item->SaveToDB(trans);
        MailDraft("장비 사용 조건 확인", "전문기술 또는 전문화 조건이 맞지 않는 장비를 보관해 드립니다.")
            .AddItem(item).SendMailTo(trans, bot, MailSender(bot, MAIL_STATIONERY_GM), MAIL_CHECK_MASK_COPIED);
        mask |= 1u << slot;
        trans->Append("INSERT INTO wowlegends_bot_equipment_repair (guid,slot,source_item) VALUES ({},{},{}) ON DUPLICATE KEY UPDATE completed=0",
                      guid, slot, itemGuid);
    }
    // Maintenance/load recovery queue retains original slot and item identity.
    if (QueryResult rows = CharacterDatabase.Query(
        "SELECT slot FROM wowlegends_bot_equipment_repair WHERE guid={} AND completed=0", guid))
    {
        do
        {
            uint8 slot = rows->Fetch()[0].Get<uint8>();
    ...
    uint32 emptyMask = 0;
    for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
        if ((mask & (1u << slot)) && !bot->GetItemByPos(INVENTORY_SLOT_BAG_0, slot))
            emptyMask |= 1u << slot;
    // This path never uses the destructive second-chance pass.
    InitEquipment(false, false, emptyMask);
    // Character/item prepared statements are registered on async connections.
    // Persist item movement, mail, new gear and queue completion in ONE atomic
    // transaction on that connection pool, never DirectCommitTransaction.
    CharacterDatabaseTransaction saved = trans;
    bot->SaveToDB(saved, false, false);
    for (uint8 slot = EQUIPMENT_SLOT_START; slot < EQUIPMENT_SLOT_END; ++slot)
        if ((mask & (1u << slot)) && bot->GetItemByPos(INVENTORY_SLOT_BAG_0, slot))
            saved->Append("UPDATE wowlegends_bot_equipment_repair SET completed=1 WHERE guid={} AND slot={}", guid, slot);
    CharacterDatabase.CommitTransaction(saved);
    LOG_INFO("playerbots", "[EquipmentRepair] bot {} checked profession gear; requested slot mask {}", guid, mask);
}
