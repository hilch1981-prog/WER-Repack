...
// WOW Legends: all-zones World PvP toggle, defined in mod-wowlegends.
extern bool WlWorldPvpEnabled();

// WOW Legends: "pitch in" toggle (grouped bots fight their own quest mobs),
// defined in mod-wowlegends.
extern bool WlBotPitchInEnabled();

// WOW Legends: "idle life" toggle (settled-master ambience for grouped
// bots), defined in mod-wowlegends.
extern bool WlBotIdleLifeEnabled();

// WOW Legends: "The Guide" toggle (escort a real player to a destination),
// defined in mod-wowlegends.
...
void AiFactory::AddDefaultCombatStrategies(Player* player, PlayerbotAI* const facade, Engine* engine)
{
    ...
    if (!player->InBattleground())
    {
        ...
        // WOW Legends: also let bots target enemy PLAYERS while in combat. The
        // 'pvp' strategy normally lives only in the non-combat engine, so a bot
        // fighting a mob can't peel onto a passing enemy player. Gated on World
        // PvP; this is config-time engine assembly (before Init), not the
        // in-tick re-entrant path.
        if (WlWorldPvpEnabled())
            engine->addStrategiesNoInit("pvp", nullptr);
    }
...
void AiFactory::AddDefaultNonCombatStrategies(Player* player, PlayerbotAI* const facade, Engine* nonCombatEngine)
{
    ...
    if (!player->InBattleground())
    {
        ...
        // WL "pitch in": player-owned bots proactively fight mobs their own
        // quest log still needs while grouped (target value stays dormant
        // without a real-player master). Added BEFORE the ChangeStrategy
        // calls below so "-pitch in" in AiPlayerbot.NonCombatStrategies can
        // veto it. Random bots keep their own grind/rpg behaviour.
        if (WlBotPitchInEnabled() && !sRandomPlayerbotMgr.IsRandomBot(player))
            nonCombatEngine->addStrategy("pitch in", false);

        // WL "idle life": settled-master ambience (strolls, campfire circle,
        // ambient emotes) - same gating rules as pitch in.
        if (WlBotIdleLifeEnabled() && !sRandomPlayerbotMgr.IsRandomBot(player))
            nonCombatEngine->addStrategy("idle life", false);

        // WL "The Guide": escort-a-player driver (dormant until the order
        // bridge sets a destination on the bot). Installed on EVERY bot -
        // including random world bots: an invited world bot accepts the
        // command and ANNOUNCES the escort either way (chat actions resolve
        // without the strategy), so without the move driver it stood there
        // like a statue after "Follow me!" (field-tested: Kneuma, Durotar,
        // 2026-07-15). Dormant = one cheap trigger check; no master, no cost.
        if (WlBotGuideEnabled())
            nonCombatEngine->addStrategy("guide", false);
    }
