...
PassiveMultiplier::PassiveMultiplier(PlayerbotAI* botAI) : Multiplier(botAI, "passive")
{
    ...
    if (allowedParts.empty())
    {
        ...
        // WL: these run only on an explicit master command, never pushed by a
        // strategy on its own - the passive stance (e.g. after flee) must not
        // silently veto a direct order to guide or hearth
        allowedParts.push_back("wl guide");
        allowedParts.push_back("hearthstone");
    }
}
