...
private:
    ...
    // AddPacket is reached from WorldSession::SendPacket on whatever thread
    // sends the packet (other maps' workers, the world thread), while Handle
    // drains on this bot's own map-update thread. The queue must be locked;
    // handlers is filled once at construction and read-only afterwards.
...
public:
    ...
    void ProcessBotOutgoingPacket(WorldPacket const& packet);
    void DrainDeferredBotOutgoingPackets();
...
private:
    ...
    // The real chat-command pipeline for the Player* overload; runs on the bot's
    // own thread (drained from HandleCommands). The public HandleCommand(Player*)
    // only enqueues into deferredChatCommands.
    void HandleCommandInternal(uint32 type, std::string const text, Player* fromPlayer);
...
protected:
    ...
    // Packets whose handling used to run inline in HandleBotOutgoingPacket on
    // the SENDING thread. They mutate AI state (AiObjectContext values, chat
    // queues, cast tracking), so they are deferred here and processed at the
    // start of UpdateAI on this bot's own map-update thread.
    std::mutex deferredBotPacketsMutex;
    std::vector<WorldPacket> deferredBotPackets;
    // Incoming chat commands used to run the full HandleCommand pipeline inline
    // on the SENDER's thread (any map worker / the world thread), mutating this
    // bot's AiObjectContext + chatCommands while it ticks its own engine - the
    // cross-thread heap-corruption fail-fast. They are enqueued here and drained
    // on this bot's own thread in HandleCommands. Store the sender GUID (not a
    // raw Player*) and re-resolve at drain time.
    ...
    std::mutex deferredChatCommandsMutex;
    std::vector<DeferredChatCommand> deferredChatCommands;
