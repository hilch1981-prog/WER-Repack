...
std::string const ChatHelper::FormatClass(Player* player, int8 spec)
{
    ...
    std::ostringstream out;
    std::string specName = specs[cls][spec];
    if (sWorld->GetDefaultDbcLocale() == LOCALE_koKR)
    {
        // TalentTab names are not loaded by this core; keep command/strategy keys unchanged.
        static std::map<std::string, std::string> const names = {
            {"balance", "조화"}, {"feral combat", "야성"}, {"restoration", "복원"},
            {"beast mastery", "야수"}, {"marksmanship", "사격"}, {"survival", "생존"},
            {"arcane", "비전"}, {"fire", "화염"}, {"frost", "냉기"}, {"holy", "신성"},
            {"protection", "방어"}, {"retribution", "징벌"}, {"discipline", "수양"},
            {"shadow", "암흑"}, {"assasination", "암살"}, {"assassination", "암살"},
            {"combat", "전투"}, {"subtlety", "잠행"}, {"elemental", "정기"},
            {"enhancement", "고양"}, {"affliction", "고통"}, {"demonology", "악마"},
            {"destruction", "파괴"}, {"arms", "무기"}, {"fury", "분노"},
            {"blood", "혈기"}, {"unholy", "부정"}
        };
        auto const name = names.find(specName);
        specName = name != names.end() ? name->second : "특성 미지정";
        if (cls == CLASS_PALADIN && spec == 1)
            specName = "보호";
    }
    out << specName << " (";
...
{
    if (auto const* entry = sChrClassesStore.LookupEntry(cls))
    {
        char const* name = entry->name[sWorld->GetDefaultDbcLocale()];
        if (name && *name)
            return name;
    }
    return classes[cls];
...
{
    if (auto const* entry = sChrRacesStore.LookupEntry(race))
    {
        char const* name = entry->name[sWorld->GetDefaultDbcLocale()];
        if (name && *name)
            return name;
