...
bool WlBotLevelFitsZone(uint32 level, uint32 zoneId);
...
#define WL_HAS_LIFE_RESERVATIONS
#endif
...
// WL: how far the ground under a scattered arrival may differ from the ground under the original
// destination before we discard the scatter. Keeps a bot from being nudged off a bridge, down a
// stairwell or onto a rooftop when the destination sits near an edge.
static constexpr float WL_ARRIVAL_SCATTER_MAX_DROP = 3.0f;
// How many candidate points to try before giving up and using the destination unchanged. Indoor
// destinations reject a fair share of candidates (walls, furniture), so one attempt would silently
// disable the scatter exactly where it is needed most.
static constexpr uint8 WL_ARRIVAL_SCATTER_TRIES = 5;
...
bool RandomPlayerbotMgr::ProcessBot(Player* bot)
{
    ...
    // Correct stale high-level bots in low-level fields without interrupting
    // players, instances, combat, camps, or participants owned by Life modules.
    bool reserved = false;
#ifdef WL_HAS_LIFE_RESERVATIONS
    reserved = PlayerbotsLife::IsReserved(botId);
#endif
    if (!reserved && IsRandomBot(bot) && !bot->GetGroup() && !bot->IsInCombat() && !bot->duel &&
        !bot->IsBeingTeleported() && bot->GetMap() && !bot->GetMap()->Instanceable() &&
        (!botAI->GetMaster() || botAI->GetMaster()->GetSession()->IsBot()) &&
        !WlBotLevelFitsZone(bot->GetLevel(), bot->GetZoneId()) &&
        !GetEventValue(botId, "zone_level_guard"))
    {
        SetEventValue(botId, "zone_level_guard", 1, 300);
...
void RandomPlayerbotMgr::RandomTeleport(Player* bot, std::vector<WorldLocation>& locs, bool hearth)
{
    ...
    for (uint32 i = 0; i < tlocs.size(); i++)
    {
        ...
        if (!WlBotLevelFitsZone(bot->GetLevel(), zone->ID))
            continue;
        ...
        // WL: the arrival scatter was left commented out upstream, so every bot sent to the same
        // destination landed on the BYTE-IDENTICAL coordinate. It shows up worst on the city-banker
        // trips (ProbTeleToBankers): a handful of bankers collected dozens of perfectly stacked bots
        // in low-traffic capitals such as Darnassus and Exodar, where no real player ever passes by
        // to wake them up again. Nudge the arrival onto a small ring around the destination.
        //
        // A bare height probe is NOT enough to decide a candidate is safe: Map::GetHeight is a
        // vertical ray, so on its own it happily approves a point on the far side of a bank wall.
        // Every candidate therefore also has to be in line of sight of the destination. Anything
        // that fails is simply retried, and if no candidate survives we keep the original spot -
        // so the worst case is exactly the old behaviour, never a bot stuffed into geometry.
        float const scatter = sPlayerbotAIConfig.wlArrivalScatterRadius;
        float const destGround = map->GetHeight(bot->GetPhaseMask(), x, y, z + 2.0f);
        if (scatter > 0.0f && destGround > INVALID_HEIGHT)
        {
            for (uint8 attempt = 0; attempt < WL_ARRIVAL_SCATTER_TRIES; ++attempt)
            {
                float const angle = frand(0.0f, 2.0f * static_cast<float>(M_PI));
                float const dist = frand(scatter * 0.35f, scatter);
                float const sx = x + dist * std::cos(angle);
                float const sy = y + dist * std::sin(angle);

                // compare ground against ground: the stored z of a banker or innkeeper spot carries
                // a lift (TravelMgr adds +2.0f), so measuring the candidate against the raw z would
                // bias every test upwards and reject good ground.
                float const sGround = map->GetHeight(bot->GetPhaseMask(), sx, sy, z + 2.0f);
                if (sGround <= INVALID_HEIGHT || std::fabs(sGround - destGround) > WL_ARRIVAL_SCATTER_MAX_DROP)
                    continue;

                if (map->IsInWater(bot->GetPhaseMask(), sx, sy, sGround + 0.05f, bot->GetCollisionHeight()))
                    continue;

                if (!map->isInLineOfSight(x, y, destGround + 2.0f, sx, sy, sGround + 2.0f, bot->GetPhaseMask(),
        ...
        // WL: arriving with a hardcoded orientation of 0 made every teleported bot face due east,
        // which together with the missing scatter turned a shared destination into an obvious
        // drill-formation of identical bots. Pick one facing and use it for the arrival and, when
        // this trip also rebinds the bot, for its new home.
        float const arrivalO = frand(0.0f, 2.0f * static_cast<float>(M_PI));
        ...
        if (hearth)
        {
            // WL: bind to the SCATTERED arrival point rather than the shared destination. Binding to
            // `loc` put hundreds of bots on one homebind coordinate - the PTR had 944 of them sharing
            // a single spot - so every death or hearth rebuilt the stack no matter how well the
            // arrival itself was spread out.
            // Also moved below the HasPlayerNearby bail-out: it used to run first, so an aborted
            // teleport still rebound the bot's home to a place it never actually went.
            WorldLocation homeLoc(loc.GetMapId(), x, y, z, arrivalO);
            bot->SetHomebind(homeLoc, zone->ID);
        }
        ...
        bot->TeleportTo(loc.GetMapId(), x, y, z, arrivalO);
...
void RandomPlayerbotMgr::RandomTeleportForLevel(Player* bot)
{
    ...
    if (bot->GetLevel() >= 10 && urand(0, 100) < sPlayerbotAIConfig.probTeleToBankers * 100)
    {
        ...
        if (!locs.empty())
        {
            // WL: this stays `true` on purpose. It looks like the culprit for bots drifting into
            // capitals, but it is the ONLY live SetHomebind path in the module - RandomTeleportForRpg
            // is dead code (its handler registration is commented out, see :2458). Passing false here
            // would leave every random bot bound to its level-1 starting zone for the rest of its
            // life, so a level 70 bot would resurrect in Elwynn Forest. The real defect was that the
            // bind landed on the shared, unscattered coordinate; that is fixed in RandomTeleport.
...
void RandomPlayerbotMgr::HandleCommand(uint32 type, std::string const text, Player* fromPlayer, std::string channelName)
{
    for (PlayerBotMap::const_iterator it = GetPlayerBotsBegin(); it != GetPlayerBotsEnd(); ++it)
    {
        ...
        // WL: guard the AI lookup (same defect as the guild-chat hook in
        // Script/Playerbots.cpp). This fan-out runs for EVERY random bot on the
        // realm on every channel line, so a single bot without an AI - one being
        // torn down, or an addclass bot whose AI was removed - took the
        // worldserver down. At PTR bot counts that is a near-certainty, not an
        // edge case.
        if (PlayerbotAI* botAI = GET_PLAYERBOT_AI(bot))
...
{
    PlayerbotFactory(bot, bot->GetLevel()).RepairProfessionEquipment();
