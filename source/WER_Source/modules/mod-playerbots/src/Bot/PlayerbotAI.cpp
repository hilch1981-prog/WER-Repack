...
{
    // Swap the queue out under the lock and process the local copy: handling a
    // packet may send packets back to this bot and re-enter AddPacket on this
    // same thread, which must not deadlock against us.
    std::stack<WorldPacket> local;
    {
...
void PacketHandlingHelper::AddPacket(WorldPacket const& packet)
{
    if (packet.empty())
        return;
    // handlers is read-only after construction, safe to check without the lock
...
void PlayerbotAI::UpdateAI(uint32 elapsed, bool minimal)
{
    ...
    // Packets deferred by HandleBotOutgoingPacket (they arrive on other threads
    // and may only be handled here, on this bot's own map-update thread).
    DrainDeferredBotOutgoingPackets();
...
{
    // Drain chat commands deferred from foreign threads (the chat hooks) and run
    // their real pipeline HERE, on this bot's own map-update thread - closing the
    // cross-thread HandleCommand race. Mirrors the deferred bot-packet drain.
    std::vector<DeferredChatCommand> localChat;
    {
        std::lock_guard<std::mutex> lock(deferredChatCommandsMutex);
        localChat.swap(deferredChatCommands);
    }
    for (auto const& cmd : localChat)
    {
        Player* from = cmd.fromGuid ? ObjectAccessor::FindPlayer(cmd.fromGuid) : nullptr;
        if (from)
            HandleCommandInternal(cmd.type, cmd.text, from);
    }
...
// Chat-prefix map (#w/#p/#r/#a/#g). Init-once const: the old global with a lazy
// `if (chatMap.empty())` populate was a cross-thread first-touch data race now
// that command handling runs on multiple bot threads. Read-only after init.
static std::map<std::string, ChatMsg> const chatMap = {
    {"#w ", CHAT_MSG_WHISPER}, {"#p ", CHAT_MSG_PARTY}, {"#r ", CHAT_MSG_RAID},
    {"#a ", CHAT_MSG_ADDON}, {"#g ", CHAT_MSG_GUILD}};
// ⚠️ WL NOTE - THIS OVERLOAD IS DEAD CODE. It is declared PRIVATE (PlayerbotAI.h:627)
// and the only call to it anywhere in the tree is its own recursion below. Every real
// caller (Script/Playerbots.cpp, PlayerbotMgr, RandomPlayerbotMgr, mod-wowlegends)
// passes a Player* and therefore binds the LIVE overload at :977 -> HandleCommandInternal.
// It has also DRIFTED from the live copy: the `lang == LANG_ADDON` drop and the "BOT\t"
// mangosbot prefix below exist ONLY here, which makes this look like working addon
// protection when the live path has none. That misread cost a real mis-diagnosis
// (2026-07-28: "bots answer my gearscore addon" was traced to this function, but the
// actual fix belongs in Script/Playerbots.cpp's OnPlayerCanUseChat hooks, where inbound
// chat still carries `lang`). Change nothing here expecting a runtime effect.
void PlayerbotAI::HandleCommand(uint32 type, const std::string& text, Player& fromPlayer, const uint32 lang)
{
    ...
    for (auto i = chatMap.begin(); i != chatMap.end(); ++i)
    {
...
void PlayerbotAI::HandleTeleportAck()
{
    ...
    if (bot->IsBeingTeleportedFar())
    {
        ...
        // Apply strategies silently after map attach; a full raid must not flood
        // the master's whispers with one automatic acknowledgement per bot.
        if (sPlayerbotAIConfig.applyInstanceStrategies)
            ApplyInstanceStrategies(bot->GetMapId(), false);
...
// Public cross-thread entry: the chat hooks call this on the SENDER's thread
// (any map worker / the world thread). Do NOT touch this bot's AI state here -
// enqueue only; HandleCommands drains it on the bot's own thread. Mirrors the
// 0001 packet-handling fix (deferredBotPackets).
void PlayerbotAI::HandleCommand(uint32 type, std::string const text, Player* fromPlayer)
{
    std::lock_guard<std::mutex> lock(deferredChatCommandsMutex);
    if (deferredChatCommands.size() < 1024)   // bound a bot stuck not updating
        deferredChatCommands.push_back(
            {text, fromPlayer ? fromPlayer->GetGUID() : ObjectGuid::Empty, type});
}

// The real chat-command pipeline; runs ONLY on the bot's own map-update thread.
void PlayerbotAI::HandleCommandInternal(uint32 type, std::string const text, Player* fromPlayer)
{
...
{
    // Called from WorldSession::SendPacket on whatever thread sends the packet
    // (other maps' worker threads, the world thread). Everything this packet
    // handling touches - AiObjectContext values, chat queues, cast tracking -
    // belongs to this bot's own map-update thread, so never process here:
    // defer to DrainDeferredBotOutgoingPackets() at the start of UpdateAI.
    if (packet.empty())
        return;
    ...
    switch (packet.GetOpcode())
    {
        ...
        case SMSG_DISMOUNT:
        {
            std::lock_guard<std::mutex> lock(deferredBotPacketsMutex);
            if (deferredBotPackets.size() < 1024)   // bound a bot stuck in a non-updating state
                deferredBotPackets.emplace_back(packet);
            return;
        }
        default:
...
void PlayerbotAI::DrainDeferredBotOutgoingPackets()
{
    std::vector<WorldPacket> local;
    {
        std::lock_guard<std::mutex> lock(deferredBotPacketsMutex);
        if (deferredBotPackets.empty())
            return;
        local.swap(deferredBotPackets);
    }

    for (WorldPacket const& packet : local)
        ProcessBotOutgoingPacket(packet);
}

void PlayerbotAI::ProcessBotOutgoingPacket(WorldPacket const& packet)
{
    if (packet.empty())
        return;
...
void PlayerbotAI::DoNextAction(bool min)
{
    ...
    if (minimal)
    {
        // WL: don't flag masterless/idle bots as AFK — they're alive (AI chat +
        // ambient chatter), so the "<Away>" tag + AFK whisper-reply was misleading.
        // The minimal-activity CPU saving (passiveDelay below) is unaffected.
...
// WOW LEGENDS: realm-wide "may bots volunteer chatter" flag, cached as an
// atomic in wowlegends_botflags.cpp and refreshed there on every config load.
// Declared the same way as every other WL leaf hook in this module - a plain
// extern, NO cross-module header - so mod-playerbots keeps building on its own.
extern bool WlBotChatterUnsolicited();
...
bool PlayerbotAI::TellMasterNoFacing(std::string const text, PlayerbotSecurityLevel securityLevel)
{
    ...
    // ---- WOW LEGENDS: the unsolicited-chatter gate --------------------------
    //
    // Community complaint, repeatedly: bots whisper a running commentary of
    // every quest they pick up, hand in or abandon, and on a realm where you
    // run five companions it buries anything you actually wanted to read.
    //
    // 🔑 THE DISCRIMINATOR IS `currentChat`, AND IT IS THE WHOLE TRICK. It is
    // set to (channel, now + 2) in HandleCommand ONLY while a bot is answering
    // something the master just said, and left at (WHISPER, 0) otherwise. So
    // it distinguishes "I am replying to you" from "I decided to tell you
    // something" - which is exactly the line players want drawn. Two seconds
    // is upstream's own window; we read it, we do not change it.
    //
    // ⚠️ Gate here and NOWHERE ELSE. There are 386 TellMaster call sites in
    // this module and this function is the single point they all funnel
    // through. Anyone "fixing" chattiness at the call sites will miss most of
    // them and break replies at the rest.
    //
    // 🛑 AND READ AN ATOMIC, NEVER sConfigMgr, ON THIS PATH. The first version
    // called sConfigMgr->GetOption right here and that is genuinely unsafe:
    // TellMasterNoFacing runs on every map-update thread (six on our realm),
    // ConfigMgr::GetValueDefault reads _configOptions WITHOUT taking
    // _configLock (Config.cpp:534), and `.reload config` clears and refills
    // that same map under the lock. Worse, the read path can call AddKey() when
    // an environment override exists - a WRITE from what looks like a getter.
    // Six threads reading a container another thread is clearing is a crash,
    // not a stale value. wowlegends_botflags.cpp pumps this flag from
    // OnAfterConfigLoad on the world thread instead, which is where every other
    // WL bot toggle is read.
    //
    // Returning false is deliberate: it is the same return the IsTellAllowed
    // rejection above uses, so every caller's suppression path is one that
    // already existed and is already exercised.
    if (currentChat.second - time(nullptr) < 1 && !WlBotChatterUnsolicited())
        return false;
    // ------------------------------------------------------------------------
...
std::vector<std::pair<Quest const*, uint32>> PlayerbotAI::GetCurrentQuestsRequiringItemId(uint32 itemId)
{
    ...
    for (uint16 slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot)
    {
        ...
        for (uint8 i = 0; i < std::size(quest->RequiredItemId); ++i)
        {
            ...
            {
                // WL fix: callers treat pair.second as the REQUIRED COUNT -
                // pushing the item ID here handed them a bogus requirement
                result.push_back(std::pair(quest, quest->RequiredItemCount[i]));
                break;
            }
        }
    }
