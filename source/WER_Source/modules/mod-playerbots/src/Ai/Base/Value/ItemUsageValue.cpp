...
{
    // WL FIX: this used to begin with
    //     PlayerbotAI* botAI = GET_PLAYERBOT_AI(player);
    //     if (!botAI) return false;
    // and that guard quietly broke the whole point of the function.
    // GetPlayerbotAI() returns non-null ONLY for actual bots (PlayerbotMgr.cpp
    // :1811 gates on IsBotAI()), so for a REAL HUMAN it always returned false.
    // The one caller that passes a human is the master check just above -
    //     masterNeedsItemForQuest = master && syncQuestWithPlayer &&
    //                               IsItemUsefulForQuest(master, proto)
    // - so "defer to the master's quest items" could never fire for a real
    // player's party. It only ever worked when the master was itself a bot,
    // which is the case nobody has. The guard was also pointless: nothing
    // below it needs a PlayerbotAI, only the player's quest log and bags.
    if (!player || !proto)
        return false;
