...
public:
    ...
    // WL: made public + static so StoreLootAction::IsLootAllowed can ask the
    // SAME question instead of carrying its own copy of the logic. The two
    // disagreeing about quest items is precisely the bug this fixed (bots
    // hoovering up a real player's quest drops while SyncQuestWithPlayer was
    // on). It touches no member state - only the player and the template.
    static bool IsItemUsefulForQuest(Player* player, ItemTemplate const* proto);
