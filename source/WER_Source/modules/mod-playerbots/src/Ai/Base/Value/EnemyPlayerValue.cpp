...
// WOW Legends: smart PvP targeting toggle, defined in mod-wowlegends.
extern bool WlSmartPvpEnabled();
// WOW Legends: world-PvP live flag + social-memory grudge, both defined in
// mod-wowlegends. A bot remembers the real player who slew it and hunts them
// first while World PvP is active.
extern bool WlWorldPvpEnabled();
extern bool WlBotSocialGrudge(uint32 botLow, uint32 playerLow);
...
Unit* EnemyPlayerValue::Calculate()
{
    ...
    // WOW Legends: smarter PvP target selection (WowLegends.SmartPvpTargeting).
    // Among the valid in-range enemies, prefer a dedicated healer, then the
    // lowest-health target, with distance as a tiebreak. When the toggle is
    // off we keep the legacy first-valid behaviour (return immediately below).
    bool const wlSmartPvp = WlSmartPvpEnabled();
    Player* bestHealer = nullptr;
    Player* bestLowHp = nullptr;
    float bestHealerDist = std::numeric_limits<float>::max();
    float bestLowHpDist = std::numeric_limits<float>::max();
    ...
    for (auto const& gTarget : players)
    {
        ...
        {
            // WOW Legends: a remembered grudge outranks every other target -
            // the bot hunts the REAL player who once slew it. Cheap atomic
            // gate first; the map lookup only runs for real players.
            if (WlWorldPvpEnabled() && !GET_PLAYERBOT_AI(pTarget) &&
                WlBotSocialGrudge(bot->GetGUID().GetCounter(),
            ...
            float const dist = bot->GetDistance(pTarget);
            if (PlayerbotAI::IsHeal(pTarget, true))
            {
                if (dist < bestHealerDist)
                {
                    bestHealer = pTarget;
                    bestHealerDist = dist;
                }
            ...
            {
                uint32 const hp = pTarget->GetHealth();
                if (hp < lowestHp || (hp == lowestHp && dist < bestLowHpDist))
                {
                    lowestHp = hp;
                    bestLowHp = pTarget;
                    bestLowHpDist = dist;
    ...
    // WOW Legends: prefer the healer, else the weakest enemy found above,
    // before falling through to the party-attacker logic.
    if (bestHealer)
        return bestHealer;
    if (bestLowHp)
        return bestLowHp;
