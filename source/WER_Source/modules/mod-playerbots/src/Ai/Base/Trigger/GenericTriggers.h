...
// WOW Legends "idle life": the (real-player) master settled down to sit and
// this grouped bot has not yet joined the circle
class WlMasterSittingTrigger : public Trigger
{
public:
    WlMasterSittingTrigger(PlayerbotAI* botAI) : Trigger(botAI, "wl master sitting", 2) {}

    bool IsActive() override;
};

// WOW Legends "The Guide": an escort destination is set on this bot
class WlGuideActiveTrigger : public Trigger
{
public:
    WlGuideActiveTrigger(PlayerbotAI* botAI) : Trigger(botAI, "wl guide active", 1) {}
