...
bool WlMasterSittingTrigger::IsActive()
{
    Player* master = botAI->GetMaster();
    if (!master || master == bot || GET_PLAYERBOT_AI(master))
        return false;

    // group + map guards BEFORE any master-state read, so no cross-map
    // player state is ever dereferenced from this bot's tick
    if (!bot->GetGroup() || bot->GetGroup() != master->GetGroup())
        return false;

    if (master->GetMapId() != bot->GetMapId() || bot->IsInCombat())
        return false;

    if (!master->IsSitState() || bot->IsSitState())
        return false;

    return true;
}

bool WlGuideActiveTrigger::IsActive()
{
