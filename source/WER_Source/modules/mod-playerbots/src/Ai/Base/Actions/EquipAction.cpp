...
void EquipAction::EquipItem(Item* item)
{
    ...
    {
        bot->SetAmmo(itemId);
        LOG_DEBUG("playerbots", "{}: {}", bot->GetName(), PlayerbotTextMgr::instance().GetBotTextOrDefault(
            "wl_equip_ammo_notice", "%item_link 탄약을 장착합니다.", {{"%item_link", chat->FormatItem(itemProto)}}));
        return;
    }
    ...
    if (!equippedBag)
    {
        ...
        if (invType == INVTYPE_RANGED || invType == INVTYPE_THROWN || invType == INVTYPE_RANGEDRIGHT)
        {
            ...
            LOG_DEBUG("playerbots", "{}: {}", bot->GetName(), PlayerbotTextMgr::instance().GetBotTextOrDefault(
                "wl_equip_ranged_notice", "%item_link 원거리 장비를 장착합니다.",
                {{"%item_link", chat->FormatItem(itemProto)}}));
            return;
        }
        ...
        if (isWeapon && canDualWieldOrTG)
        {
            ...
            if (canGoMain && betterThanMH && mhConditionOK)
            {
                ...
                if (mainHandItem && mainHandCanGoOff && (!offHandItem || mainHandScore > offHandScore))
                {
                    ...
                    LOG_DEBUG("playerbots", "{}: {}", bot->GetName(), PlayerbotTextMgr::instance().GetBotTextOrDefault(
                        "wl_equip_move_offhand_notice", "주장비를 교체하고 %item_link 보조장비를 장착합니다.",
                        {{"%item_link", chat->FormatItem(oldMHProto)}}));
                }

                LOG_DEBUG("playerbots", "{}: {}", bot->GetName(), PlayerbotTextMgr::instance().GetBotTextOrDefault(
                    "wl_equip_mainhand_notice", "%item_link 주장비를 장착합니다.",
                    {{"%item_link", chat->FormatItem(itemProto)}}));
                return;
            }
            ...
            else if (canGoOff && newItemScore > offHandScore)
            {
                ...
                LOG_DEBUG("playerbots", "{}: {}", bot->GetName(), PlayerbotTextMgr::instance().GetBotTextOrDefault(
                    "wl_equip_offhand_notice", "%item_link 보조장비를 장착합니다.",
                    {{"%item_link", chat->FormatItem(itemProto)}}));
                return;
            }
            else
    ...
    LOG_DEBUG("playerbots", "{}: {}", bot->GetName(), PlayerbotTextMgr::instance().GetBotTextOrDefault(
        "wl_equip_item_notice", "%item_link 장비를 장착합니다.", {{"%item_link", chat->FormatItem(itemProto)}}));
}
