...
bool DebugAction::Execute(Event event)
{
    ...
    else if (text.find("travel ") != std::string::npos)
    {
        ...
        {
            botAI->TellMasterNoFacing("목적지 " + destination + " not found.");
            return true;
        }
    }
    else if (text.find("quest ") != std::string::npos)
    {
        ...
        if (!quest)
        {
            botAI->TellMasterNoFacing("퀘스트 " + text.substr(6) + " not found.");
            return false;
        }
    ...
    else if (text.find("add node") != std::string::npos)
    {
        ...
        botAI->TellMasterNoFacing("경로 지점 " + name + " created.");
    ...
    else if (text.find("rem node") != std::string::npos)
    {
        ...
        {
            botAI->TellMasterNoFacing("이 경로 지점은 제거할 수 없습니다.");
        }

        TravelNodeMap::instance().m_nMapMtx.lock();
        TravelNodeMap::instance().removeNode(startNode);
        botAI->TellMasterNoFacing("경로 지점을 제거했습니다.");
