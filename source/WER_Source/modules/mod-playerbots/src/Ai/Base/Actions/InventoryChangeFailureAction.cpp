...
bool InventoryChangeFailureAction::Execute(Event event)
{
    ...
    if (messages.empty())
    {
        ...
        messages[EQUIP_ERR_CANT_EQUIP_WITH_TWOHANDED] = PlayerbotTextMgr::instance().GetBotTextOrDefault(
            "wl_equip_twohand_conflict", "양손 무기를 착용 중이라 해당 장비를 함께 착용할 수 없습니다.", {});
    ...
    if (!msg.empty())
    {
        // Hide only this repetitive equipment conflict; other inventory errors stay visible.
        if (err == EQUIP_ERR_CANT_EQUIP_WITH_TWOHANDED)
        {
            LOG_DEBUG("playerbots", "{}: {}", bot->GetName(), msg);
            return true;
        }
