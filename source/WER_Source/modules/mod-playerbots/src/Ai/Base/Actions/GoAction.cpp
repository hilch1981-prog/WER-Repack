...
bool GoAction::Execute(Event event)
{
    ...
    if (param.find("travel") != std::string::npos && param.size() > 7)
    {
        ...
        {
            botAI->TellMasterNoFacing("이동 목표를 초기화합니다.");
    ...
    botAI->TellMaster("귓속말로 go x,y, go [게임 오브젝트], go unit 또는 go position을 보내면 그곳으로 갈게요.");
    return false;
}
