...
float MovementAction::MoveDelay(float distance, bool backwards)
{
    ...
    // WL: a WALK-mode spline (idle-life strolls, master-walk mirror) covers
    // the leg at walk speed - a RUN-based delay would expire the "last
    // movement" shield a third of the way in and let follow yank the bot
    else if (bot->m_movementInfo.HasMovementFlag(MOVEMENTFLAG_WALKING))
    {
        speed = bot->GetSpeed(MOVE_WALK);
    }
    else
    {
...
bool DisperseSetAction::Execute(Event event)
{
    ...
    if (text == "disable")
    {
        ...
        botAI->TellMasterNoFacing("분산 이동을 해제합니다.");
        return true;
    }
