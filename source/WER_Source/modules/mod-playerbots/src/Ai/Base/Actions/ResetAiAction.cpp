...
bool ResetAiAction::Execute(Event event)
{
    ...
    // WL: a reset also ends an active guide escort (values live outside the
    // strategy set and would otherwise survive as dormant hidden state)
    context->GetValue<std::string>("wl guide label")->Set("");

    botAI->TellMaster("AI 설정이 기본값으로 초기화되었습니다.");
    return true;
}
