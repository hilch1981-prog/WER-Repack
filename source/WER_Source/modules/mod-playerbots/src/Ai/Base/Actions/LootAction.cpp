...
bool StoreLootAction::IsLootAllowed(uint32 itemid, PlayerbotAI* botAI)
{
    ...
    for (uint8 slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot)
    {
        ...
        for (uint8 i = 0; i < 4; i++)
        {
            ...
            {
                // WL FIX (2026-08-01, reported by DMCraig - 1 real player, 4
                // bots, everyone forced to loot every quest item by hand).
                //
                // playerbots.conf promises that with SyncQuestWithPlayer on,
                // bots "will not loot quest items" - and they genuinely do not
                // need them, because their copy of the quest completes the
                // moment the PLAYER hands it in. ItemUsageValue already honours
                // that. This function did not: the deferral below it was
                // commented out, so it returned true and looted anyway. Two
                // paths, opposite answers, and this one wins.
                //
                // Restored using ItemUsageValue's own test rather than a second
                // copy of the logic - having two versions of "does the master
                // need this?" is how they drifted apart in the first place.
                //   - isSelfBot: a player running their OWN character as a bot
                //     is its own master and must still loot for itself.
                //   - IsItemUsefulForQuest already stops deferring once the
                //     master has enough of the item, so the bot is free to take
                //     the surplus.
                Player* master = botAI->GetMaster();
                bool const isSelfBot = (master == botAI->GetBot());
                if (master && !isSelfBot && sPlayerbotAIConfig.syncQuestWithPlayer &&
                    ItemUsageValue::IsItemUsefulForQuest(master, proto))
                    return false;
