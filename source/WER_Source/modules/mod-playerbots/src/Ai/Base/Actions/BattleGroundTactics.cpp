...
bool BGTactics::HandleConsoleCommand(ChatHandler* handler, char const* args)
{
    ...
    if (!session)
    {
        handler->PSendSysMessage("접속 중인 캐릭터로 사용해야 하는 명령어입니다.");
        return true;
    }
    ...
    if (target)
    {
        ...
        if (losBlocked)
        {
            ...
            if (path.GetPathType() != PATHFIND_NOPATH)
            {
                ...
                botAI->TellMasterNoFacing("대상의 시야에서 벗어날 위치로 이동합니다!");
...
bool ArenaTactics::moveToCenter(Battleground* bg)
{
    ...
    switch (bg->GetBgTypeID())
    {
        ...
        case BATTLEGROUND_RV:
            // WL: bots used to set off for the centre the instant this ran - while
            // still riding the starting elevator. Pathing off a moving lift walks
            // them into the pit, so they arrive alone and die before the player is
            // even up. The core lifts anything below Z 27 onto the arena floor when
            // the fences open (BattlegroundRV.cpp:96, elevators rise in ~20.1s), so
            // below that height the only correct move is to stand still and ride.
