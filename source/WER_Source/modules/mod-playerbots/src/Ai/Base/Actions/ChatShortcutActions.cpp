...
bool FollowChatShortcutAction::Execute(Event /*event*/)
{
    ...
    // WL: "follow" cancels an active guide escort (own tick, own values)
...
bool StayChatShortcutAction::Execute(Event /*event*/)
{
    ...
    // WL: "stay" cancels an active guide escort (own tick, own values)
    context->GetValue<std::string>("wl guide label")->Set("");

    SetReturnPosition(bot->GetPositionX(), bot->GetPositionY(), bot->GetPositionZ());
    SetStayPosition(bot->GetPositionX(), bot->GetPositionY(), bot->GetPositionZ());

    // A raid-wide stay command must not send one success whisper per bot.
    // Keep the actual reset, strategies and saved positions above unchanged.
    return true;
}
...
bool NaxxChatShortcutAction::Execute(Event /*event*/)
{
    ...
    botAI->TellMasterNoFacing("낙스라마스 공략 행동을 추가합니다!");
...
bool BwlChatShortcutAction::Execute(Event /*event*/)
{
    ...
    botAI->TellMasterNoFacing("검은날개 둥지 공략 행동을 추가합니다!");
    return true;
}
