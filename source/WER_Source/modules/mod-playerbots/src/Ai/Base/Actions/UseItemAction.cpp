...
bool UseItemAction::UseItem(Item* item, ObjectGuid goGuid, Item* itemTarget, Unit* unitTarget)
{
    ...
    if (item->GetTemplate()->Stackable > 1)
    {
        ...
        if (count > 1)
        itemText += " (" + std::to_string(count) + "개 보유)";
        else
            itemText += " (마지막 1개)";
    }
    ...
    if (uint32 questid = item->GetTemplate()->StartQuest)
    {
        if (Quest const* qInfo = sObjectMgr->GetQuestTemplate(questid))
        {
            ...
            botAI->TellMasterNoFacing("퀘스트를 받았습니다: " + chat->FormatQuest(qInfo));
            return true;
        }
    }
        ...
        (proto->SubClass == ITEM_SUBCLASS_FOOD || proto->SubClass == ITEM_SUBCLASS_CONSUMABLE) && (isFood || isDrink))
    {
        ...
        {
            p = std::min(hp, mp);
            TellConsumableUse(item, "음식과 음료 사용", p);
        }
        else if (isDrink)
        {
            p = mp;
            TellConsumableUse(item, "음료 사용", p);
        }
        else if (isFood)
        {
            p = std::min(hp, mp);
            TellConsumableUse(item, "음식 사용", p);
        }
    ...
    // botAI->SetNextCheckDelay(sPlayerbotAIConfig.globalCoolDown);
    // Automatic item use is already visible through the normal combat log.
    // Do not duplicate it as ordinary master chat; this path also covers
    // targeted recovery items and injectors, not only self-use.
    bot->GetSession()->HandleUseItemOpcode(packet);
    return true;
}
