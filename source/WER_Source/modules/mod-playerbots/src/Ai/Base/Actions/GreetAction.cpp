...
bool GreetAction::Execute(Event /*event*/)
{
    ...
    // Record nearby players even if this bot stays quiet. Otherwise a failed
    // chance roll would be retried every trigger tick in a crowded area.
    ...
    for (ObjectGuid const nearbyGuid : nearestPlayers)
        alreadySeenPlayers.insert(nearbyGuid);

    if (!roll_chance_i(static_cast<int32>(sPlayerbotAIConfig.greetChance)))
        return false;
