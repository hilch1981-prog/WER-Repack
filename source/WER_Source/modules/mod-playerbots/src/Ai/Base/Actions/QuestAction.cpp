...
// WOW Legends "need narration": grouped bots SAY short human lines about
// their own quest progress in PARTY chat ("Still need 2 more Plainstrider").
// Rides the bot's own progress packets (available/required arrive
// pre-computed); lines are DB-editable via ai_playerbot_texts
// (wl_narrate_* keys, several rows per key = variety) exactly like the
// taunts. One shared per-bot cooldown keeps it charming, not chatty.
// The toggle lives in mod_wowlegends.conf (extern - same pattern as
// WlWorldPvpEnabled).
extern bool WlBotNarrationEnabled();

namespace
{
    void WlNarrateToParty(PlayerbotAI* botAI, Player* bot, std::string const& key,
        std::string const& defaultText,
        std::map<std::string, std::string> const& placeholders)
    {
        if (!WlBotNarrationEnabled())
            return;
        if (!bot->GetGroup())
            return;
        ...
        time_t const nextAllowed = context->GetValue<time_t>("last said", "wl_narrate")->Get();
        if (now < nextAllowed)
            return;
        context->GetValue<time_t>("last said", "wl_narrate")->Set(now + 25 + urand(0, 20));

        // one narrator per shared event: push the OTHER bot members' clocks
        // forward (raise only) so a group kill yields one line, not N
        // near-identical ones - same group stagger SayAction uses
        int index = 0;
        for (GroupReference* ref = bot->GetGroup()->GetFirstMember(); ref; ref = ref->next())
        {
            ...
            time_t const memberNext = now + 20 * ++index + urand(1, 15);
            Value<time_t>* memberLastSaid =
                memberAi->GetAiObjectContext()->GetValue<time_t>("last said", "wl_narrate");
            if (memberLastSaid->Get() < memberNext)
                memberLastSaid->Set(memberNext);
        }

        std::string const text =
            PlayerbotTextMgr::instance().GetBotTextOrDefault(key, defaultText, placeholders);
        if (!text.empty())
...
bool QuestAction::CompleteQuest(Player* player, uint32 entry)
{
    ...
    if (botAI->HasStrategy("debug quest", BotState::BOT_STATE_NON_COMBAT) || botAI->HasStrategy("debug rpg", BotState::BOT_STATE_COMBAT))
    {
        ...
        bot->Say("퀘스트 [ " + text_quest + " ] completed", LANG_UNIVERSAL);
    }
    botAI->TellMasterNoFacing("퀘스트 완료: " + text_quest);
...
bool QuestUpdateCompleteAction::Execute(Event event)
{
    ...
    if (qInfo)
    {
        ...
        if (botAI->GetMaster())
            botAI->TellMasterNoFacing("퀘스트 완료: " + format);
        BroadcastHelper::BroadcastQuestUpdateComplete(botAI, bot, qInfo);
        std::map<std::string, std::string> narratePh;
        narratePh["%quest_link"] = format;
        WlNarrateToParty(botAI, bot, "wl_narrate_quest_done", "Done with mine!", narratePh);
...
bool QuestUpdateAddKillAction::Execute(Event event)
{
    ...
    if (qInfo && (entry & 0x80000000))
    {
        ...
        if (info)
        {
            ...
            std::map<std::string, std::string> narratePh;
            narratePh["%quest_obj_name"] = infoName;
            narratePh["%quest_obj_missing"] =
                std::to_string(required > available ? required - available : 0);
            narratePh["%quest_link"] = ChatHelper::FormatQuest(qInfo);
            if (available < required)
                WlNarrateToParty(botAI, bot, "wl_narrate_progress",
                    "Still need %quest_obj_missing more %quest_obj_name.", narratePh);
            else
                WlNarrateToParty(botAI, bot, "wl_narrate_objective_done",
                    "That's all the %quest_obj_name I needed.", narratePh);
        }
    }
    else if (qInfo)
    {
    ...
    for (uint16 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
        ...
        for (int i = 0; i < QUEST_ITEM_OBJECTIVES_COUNT; i++)
        {
            ...
            if (itemId == itemEntry && uint32(previousCount) < quest->RequiredItemCount[i])
            {
                ...
                std::map<std::string, std::string> narratePh;
                narratePh["%item_link"] = itemLink;
                narratePh["%quest_obj_missing"] =
                    std::to_string(required > available ? required - available : 0);
                narratePh["%quest_link"] = ChatHelper::FormatQuest(quest);
                if (available < required)
                    WlNarrateToParty(botAI, bot, "wl_narrate_need_item",
                        "Still need %quest_obj_missing more %item_link.", narratePh);
                else
                    WlNarrateToParty(botAI, bot, "wl_narrate_item_done",
                        "Got all the %item_link I need.", narratePh);
            }
        }
    }
...
bool QuestUpdateFailedTimerAction::Execute(Event event)
{
    ...
    {
        botAI->TellMaster("시간 제한 실패: " + std::to_string(questId));
    }
