...
// WOW Legends "idle life" (toggle in mod-wowlegends; predicate in
// ChooseTargetActions.cpp)
extern bool WlBotIdleLifeEnabled();
extern bool WlIdleLifeMasterSettled(PlayerbotAI* botAI, Player* bot);
...
bool FollowAction::isUseful()
{
    ...
    // WL "idle life": while the master is SETTLED, a bot that is already
    // home (or seated) is released from its formation slot so strolls and
    // the campfire circle are not yanked back to the slot. The settled
    // predicate collapses the same tick the master moves/mounts/fights, so
    // follow resumes instantly.
    if (WlBotIdleLifeEnabled()
        && botAI->HasStrategy("idle life", BOT_STATE_NON_COMBAT)
        && WlIdleLifeMasterSettled(botAI, bot))
    {
        Player* master = botAI->GetMaster();
        if (master && (bot->IsSitState() || bot->GetDistance(master) < 12.0f))
            return false;
    }
...
bool FleeToGroupLeaderAction::Execute(Event /*event*/)
{
    ...
    {
        if (!urand(0, 3))
            botAI->TellMaster("거의 다 왔어요. 조금만 기다려 주세요!");
    }
    else if (distance < 1000)
    {
        if (!urand(0, 10))
            botAI->TellMaster("지금 계신 곳으로 갈게요.");
    }
    else if (!urand(0, 20))
        botAI->TellMaster("지금 계신 곳으로 이동 중이에요.");
