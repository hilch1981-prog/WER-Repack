...
namespace
{
std::string LocalizedSuggestionName(std::string name, bool faction)
{
    static std::map<std::string, std::string> const aliases = {
        {"Deadmines", "The Deadmines"}, {"Stockade", "The Stockade"},
        {"Scarlet Monestery", "Scarlet Monastery"}, {"Strathholme", "Stratholme"},
        {"Hellfire Citidel", "Hellfire Citadel"}, {"Cavens of Time", "Caverns of Time"},
        {"Lower Blackrock Spire", "Blackrock Spire"}, {"Violet Hold", "The Violet Hold"},
        {"Oculus", "The Oculus"}, {"Forge of Souls", "The Forge of Souls"},
        {"Temple of Atal'Hakkar", "Sunken Temple"},
        {"Ashen Verdict", "The Ashen Verdict"}
    };
    auto const alias = aliases.find(name);
    if (alias != aliases.end())
        name = alias->second;

    // Resolve the English selection keys once; display names come from the loaded locale.
    static auto const factionIds = []
    {
        std::map<std::string, uint32> ids;
        for (uint32 i = 0; i < sFactionStore.GetNumRows(); ++i)
            if (auto const* entry = sFactionStore.LookupEntry(i))
                ids.emplace(entry->name[LOCALE_enUS], i);
        return ids;
    }();
    static auto const mapIds = []
    {
        std::map<std::string, uint32> ids;
    ...
    static auto const areaIds = []
    {
        std::map<std::string, uint32> ids;
        for (uint32 i = 0; i < sAreaTableStore.GetNumRows(); ++i)
            if (auto const* entry = sAreaTableStore.LookupEntry(i))
                ids.emplace(entry->area_name[LOCALE_enUS], i);
        return ids;
    }();
    LocaleConstant const locale = sWorld->GetDefaultDbcLocale();
    char const* localized = nullptr;
    if (faction)
    {
        auto const found = factionIds.find(name);
        if (found != factionIds.end())
            localized = sFactionStore.LookupEntry(found->second)->name[locale];
    }
    else
    {
        auto const map = mapIds.find(name);
        if (map != mapIds.end())
            localized = sMapStore.LookupEntry(map->second)->name[locale];
        if (!localized || !*localized)
        {
            auto const area = areaIds.find(name);
            if (area != areaIds.end())
                localized = sAreaTableStore.LookupEntry(area->second)->area_name[locale];
        }
    }
    if (localized && *localized)
        return localized;
    return locale == LOCALE_koKR ? (faction ? "해당 평판" : "해당 던전") : name;
}
}
...
void SuggestWhatToDoAction::grindReputation()
{
    ...
    bool const korean = sWorld->GetDefaultDbcLocale() == LOCALE_koKR;
    levels.push_back(korean ? "우호적" : "honored");
    levels.push_back(korean ? "매우 우호적" : "revered");
    levels.push_back(korean ? "확고한 동맹" : "exalted");

    std::vector<std::string> allowedFactions;
    for (auto it : factions)
    {
        if (bot->GetLevel() >= it.second)
            allowedFactions.push_back(LocalizedSuggestionName(it.first, true));
    }
...
bool SuggestDungeonAction::Execute(Event /*event*/)
{
    ...
    {
        if (bot->GetLevel() >= it.second)
            allowedInstances.push_back(LocalizedSuggestionName(it.first, false));
    }
