...
namespace ai::buff
{
    namespace
    {
        ...
        std::string BuffDisplayName(PlayerbotAI* botAI, std::string const& internalName)
        {
            LocaleConstant const locale = sWorld->GetDefaultDbcLocale();
            uint32 const spellId = botAI->GetAiObjectContext()->GetValue<uint32>("spell id", internalName)->Get();
            ...
            // Never replace AI lookup keys or DBC data; this is display text only.
            return locale == LOCALE_koKR ? "해당 강화 주문" : internalName;
        }
        ...
        PlayerbotAI* botAI, std::string const& baseName, std::string const& groupName)
    {
        ...
        std::map<std::string, std::string> placeholders = {
            {"%base_spell", BuffDisplayName(botAI, baseName)},
            {"%group_spell", BuffDisplayName(botAI, groupName)}
        };
