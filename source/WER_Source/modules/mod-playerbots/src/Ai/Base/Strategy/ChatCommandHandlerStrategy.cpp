...
void ChatCommandHandlerStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    // WL: strategy toggles outrank other queued chat commands. A vetoed
    // action still consumes its trigger, so a same-batch "co -passive" wake
    // must always pop before the combat order it unblocks (queued baskets
    // survive the extra ticks - AiPlayerbot.ExpireActionTime).
    triggers.push_back(new TriggerNode("co", { NextAction("co", relevance + 1) }));
    triggers.push_back(new TriggerNode("nc", { NextAction("nc", relevance + 1) }));
...
ChatCommandHandlerStrategy::ChatCommandHandlerStrategy(PlayerbotAI* botAI) : PassThroughStrategy(botAI)
{
    ...
    // WL: "co"/"nc" moved to explicit InitTriggers nodes at relevance + 1
    ...
    // WL: "$hearthstone" = actually USE the hearthstone (the "home" command
    // only SETS the hearth at a nearby innkeeper); action exists upstream
    // (ActionContext "hearthstone" -> UseHearthStone), it just had no
    // chat command wired to it.
    supported.push_back("hearthstone");
    // WL "The Guide": "$wl guide <mapId> <x> <y> <z> <label>" / "$wl guide
    // stop" - dispatched by the order bridge, params ride the command text
    supported.push_back("wl guide");
