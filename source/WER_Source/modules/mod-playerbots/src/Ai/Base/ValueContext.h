...
public:
    ValueContext()
    {
        ...
        creators["pitch in target"] = &ValueContext::pitch_in_target;
        creators["wl guide pos"] = &ValueContext::wl_guide_pos;
        creators["wl guide label"] = &ValueContext::wl_guide_label;
        creators["wl guide ztol"] = &ValueContext::wl_guide_ztol;
        creators["wl guide seq"] = &ValueContext::wl_guide_seq;
...
private:
    ...
    static UntypedValue* pitch_in_target(PlayerbotAI* botAI) { return new WlPitchInTargetValue(botAI); }
    // WL "The Guide": escort destination, written ONLY from the bot's own
    // tick (WlGuideAction parses it out of the chat-command text)
    static UntypedValue* wl_guide_pos(PlayerbotAI* botAI) { return new ManualSetValue<WorldPosition>(botAI, WorldPosition(), "wl guide pos"); }
    static UntypedValue* wl_guide_label(PlayerbotAI* botAI) { return new ManualSetValue<std::string>(botAI, "", "wl guide label"); }
    static UntypedValue* wl_guide_ztol(PlayerbotAI* botAI) { return new ManualSetValue<uint32>(botAI, 0, "wl guide ztol"); }
    // bumped on every ACCEPTED guide command so the move action can tell a
    // brand-new escort from a repeat of the same destination coords
    static UntypedValue* wl_guide_seq(PlayerbotAI* botAI) { return new ManualSetValue<uint32>(botAI, 0, "wl guide seq"); }
