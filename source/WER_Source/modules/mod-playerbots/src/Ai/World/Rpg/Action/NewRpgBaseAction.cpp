...
bool NewRpgBaseAction::MoveFarTo(WorldPosition dest, bool allowTeleport, bool* outStuck)
{
    if (dest == WorldPosition() || !dest.IsPositionValid() ||
        dest.GetPositionZ() == INVALID_HEIGHT || dest.GetPositionZ() == VMAP_INVALID_HEIGHT_VALUE)
        return false;
    ...
    else if (++botAI->rpgInfo.stuckAttempts >= 5 && GetMSTimeDiffToNow(botAI->rpgInfo.stuckTs) >= stuckTime)
    {
        ...
        // WL guide mode: never teleport - report the give-up instead
        if (!allowTeleport)
        {
            if (outStuck)
                *outStuck = true;
    ...
    // WL "Legend Roads": long same-map travel prefers the precomputed
    // slope-capped pathway network - a globally planned route instead of
    // greedy navmesh legs, so bots walk around mountains like players do.
    // No coverage (instances, islands, unbuilt maps) falls through to the
    // legacy logic below; the stuck/teleport recovery above still guards a
    // route that stops making progress.
    if (WlBotPathwaysEnabled() && WlBotPathwaysAllBots()
        && dest.GetMapId() == bot->GetMapId())
    {
        if (pwDest != dest)
        {
            ...
            pwIdx = 0;
            pwRoute.clear();
            WlPathwaysRoute(bot->GetMapId(),
                bot->GetPositionX(), bot->GetPositionY(), bot->GetPositionZ(),
                dest.GetPositionX(), dest.GetPositionY(), dest.GetPositionZ(), pwRoute);
        }
        if (!pwRoute.empty())
        {
            // z-checked like the guide's consume loop: a street node is not
            // "reached" from the bridge deck above it
            bool consumed = false;
            while (pwIdx < pwRoute.size()
                   && bot->GetExactDist2d(pwRoute[pwIdx][0], pwRoute[pwIdx][1]) < 12.0f
                   && std::fabs(bot->GetPositionZ() - pwRoute[pwIdx][2]) < 25.0f)
            {
                ++pwIdx;
                consumed = true;
            }
            if (consumed)
            {
                // consuming a route node IS progress: a legit road detour
                // (rim walls, around-water routes) can hold crow-fly distance
                // flat for minutes. Re-seed so teleport recovery only fires
                // when the route itself stops being consumed.
                botAI->rpgInfo.nearestMoveFarDis = disToDest;
                botAI->rpgInfo.stuckTs = getMSTime();
                botAI->rpgInfo.stuckAttempts = 0;
            }
            if (pwIdx < pwRoute.size())
            {
                auto const& n = pwRoute[pwIdx];
                // first node may sit up to the 180yd entry-search radius
                // away (the bot walks to the road); mid-route keeps the
                // tighter port/teleport guard
                float const offRouteMax = pwIdx == 0 ? 190.0f : 150.0f;
                if (bot->GetExactDist(n[0], n[1], n[2]) < offRouteMax)
                    return MoveTo(bot->GetMapId(), n[0], n[1], n[2], false, false, false, true);
                pwDest = WorldPosition(); // far off route (ported?) - re-plan next call
            }
            else if (dis > 150.0f)
            {
                // consumed route from an EARLIER trip to this dest while we
                // are far away again - plan a fresh road next call
                pwDest = WorldPosition();
            }
            // route consumed or abandoned: the legacy logic finishes the job
        }
    }
    ...
    // WL guide mode never reaches the random cone below (WlGuideMoveAction
    // owns its own deterministic stepping logic); report the block instead.
    if (!allowTeleport)
    {
        if (outStuck)
            *outStuck = true;
