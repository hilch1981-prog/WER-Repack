...
void GenericPaladinStrategy::InitTriggers(std::vector<TriggerNode*>& triggers)
{
    ...
    // WL: removed the "protect party member" -> Blessing of Protection node.
    // Auto-BoP is uniquely harmful: on the tank it wipes all melee threat, and
    // it forces Forbearance (blocking the target's own BoP/Divine Shield/Lay on
    // Hands for 2 min) and stops a melee target attacking. Paladins already
    // save an endangered ally cleanly via "lay on hands on party" (a full heal,
    // no threat/Forbearance cost). The un-stubbed protect value now drives only
    // the safe cast-in-place externals (Priest Pain Suppression) + non-tank
    // warrior Intervene. (Hand of Sacrifice, the clean paladin external, is a
    // possible future add.)
