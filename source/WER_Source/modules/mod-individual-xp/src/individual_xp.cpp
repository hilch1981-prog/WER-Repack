...
{
    bool Enabled, AnnounceModule, AnnounceRatesOnLogin, BotUseOwnerRate;
...
public:
    ...
    void OnBeforeConfigLoad(bool /*reload*/) override
    {
        ...
        individualXp.BotUseOwnerRate = sConfigMgr->GetOption<bool>("IndividualXp.BotUseOwnerRate", true);
    }
};
...
public:
    ...
    void OnPlayerGiveXP(Player* player, uint32& amount, Unit* /*victim*/, uint8 /*xpSource*/) override
    {
        if (individualXp.Enabled)
        {
            Player* rateOwner = player;
#if __has_include("PlayerbotAI.h")
            if (individualXp.BotUseOwnerRate && player->GetSession()->IsBot())
                if (PlayerbotAI* ai = GET_PLAYERBOT_AI(player))
                    if (Player* master = ai->GetMaster(); master && master->GetSession() && !master->GetSession()->IsBot())
                        rateOwner = master;
#endif
            if (PlayerXpRate* data = rateOwner->CustomData.Get<PlayerXpRate>("IndividualXP"))
            {
