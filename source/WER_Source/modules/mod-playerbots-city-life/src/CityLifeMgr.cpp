...
namespace CityLife
{
    ...
    {
        handler->PSendSysMessage("도시 생활 명령어:");
    ...
    bool Manager::HandleHubCommand(ChatHandler* handler, std::vector<std::string> const& args)
    {
        ...
        if (operation == "add")
        {
            ...
            {
                handler->PSendSysMessage("도시 생활: 진영, 레벨, 인원 또는 우선순위가 올바르지 않습니다.");
                return true;
            }
            ...
            LoadData();
            handler->PSendSysMessage("도시 생활: GM 위치에 {} 거점을 저장했습니다.", name);
            return true;
        }
        ...
        if ((operation == "enable" || operation == "disable") && args.size() >= 3)
        {
            ...
            LoadData();
            handler->PSendSysMessage("도시 생활: {} {}.", name, operation == "enable" ? "enabled" : "disabled");
            return true;
        }
        ...
        if (operation == "population" && args.size() >= 4)
        {
            ...
            LoadData();
            handler->PSendSysMessage(
                "도시 생활: {} 거점의 DB 인원을 {}명으로 설정했습니다. 설정 파일에 지정한 값이 있으면 우선합니다.",
                name, population);
            return true;
        }
        return false;
    ...
    bool Manager::HandleSpotCommand(ChatHandler* handler, std::vector<std::string> const& args)
    {
        ...
        if (!hub)
        {
            handler->PSendSysMessage("도시 생활: '{}' 거점이 없습니다.", args[2]);
            return true;
        }

        if (operation == "list")
        {
            handler->PSendSysMessage("도시 생활 - {} 지점 목록:", hub->Name);
            for (Spot const& spot : hub->Spots)
                handler->PSendSysMessage("#{} {} [{}] 가중치={} 반경={} ({}, {}, {})",
        ...
        if (operation == "add")
        {
            ...
            LoadData();
            handler->PSendSysMessage("도시 생활: GM 위치에 {}/{} 지점을 저장했습니다.", hub->Name, spotName);
            return true;
        }
        ...
        if (operation == "delete" && args.size() >= 4)
        {
            ...
            WorldDatabase.DirectExecute("DELETE FROM city_life_spot WHERE hub_id={} AND name='{}'",
                hub->Id, spotName);
            LoadData();
            handler->PSendSysMessage("도시 생활: {}/{} 지점을 삭제했습니다.", hub->Name, spotName);
            return true;
        }
        return false;
    }
    ...
    bool Manager::HandleCommand(ChatHandler* handler, std::string const& text)
    {
        ...
        if (command == "reload")
        {
            if (!_databaseReady || !LoadData())
                handler->PSendSysMessage("도시 생활: 다시 불러오지 못했습니다. DB와 서버 로그를 확인하세요.");
            else
                handler->PSendSysMessage("도시 생활: 거점과 지점 데이터를 다시 불러왔습니다.");
            return true;
        }
