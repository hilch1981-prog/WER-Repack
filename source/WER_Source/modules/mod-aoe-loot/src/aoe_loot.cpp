...
bool AOELootServer::CanPacketReceive(WorldSession* session, WorldPacket const& packet)
{
    ...
    // Validate the selected corpse before moving any neighbouring loot.
    // Player::SendLoot checks distance later, after this packet hook returns.
    if (mainCreature->IsAlive() ||
        !mainCreature->IsWithinDistInMap(player, INTERACTION_DISTANCE) ||
        !player->isAllowedToLoot(mainCreature))
        return true;
    ...
    // WL: keep the WOW Legends 1.5.3 corpse cap on top of upstream #66.
    // Upstream no longer limits how many corpses one loot click merges; WOW
    // Legends shipped a cap of 10 "to improve stability". AOELoot.MaxCorpses
    // is honoured (1..50, default 10).
    size_t maxCorpses = sConfigMgr->GetOption<uint32>("AOELoot.MaxCorpses", 10);
    if (maxCorpses < 1)
        maxCorpses = 1;
    if (maxCorpses > 50)
        maxCorpses = 50;
    ...
    for (Creature* creature : nearbyCorpses)
    {
        ...
        // WL: corpse cap (see above). Unprocessed corpses keep their loot.
        if (processedCreatures.size() >= maxCorpses)
            break;
        ...
        for (size_t i = 0; i < loot->quest_items.size(); ++i)
        {
            ...
            if (questItem.is_looted || !player->HasQuestForItem(questItem.itemid))
                continue;
