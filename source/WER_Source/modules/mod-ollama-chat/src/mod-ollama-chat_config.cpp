...
namespace
{
    bool ollamaEnableConfigured = false;
    bool ollamaRuntimeStarted = false;
}
...
void LoadOllamaChatConfig()
{
    ...
    // Starting/stopping workers and loading schema is a lifecycle transition,
    // not an endpoint reload. Require a restart when Enable changes.
    bool requestedEnable = sConfigMgr->GetOption<bool>("OllamaChat.Enable", true);
    if (ollamaEnableConfigured && requestedEnable != g_Enable)
        LOG_WARN("module.ollamachat", "[Ollama Chat] Changing OllamaChat.Enable requires a server restart; retaining current state.");
    else
        g_Enable = requestedEnable;
    ollamaEnableConfigured = true;
...
void OllamaChatConfigWorldScript::OnStartup()
{
    ...
    if (!g_Enable)
    {
        LOG_INFO("module.ollamachat", "[Ollama Chat] Disabled: skipping database loaders, capability probe and workers.");
        return;
    }
    ollamaRuntimeStarted = true;
