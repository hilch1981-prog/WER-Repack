...
public:
    ...
    static bool HandleAddTransmogItemSet(ChatHandler* handler, Optional<PlayerIdentifier> player, Variant<Hyperlink<itemset>, uint32> itemSetId)
    {
        ...
        if (isNotConsole)
        {
            ...
            if (!added)
            {
                handler->PSendSysMessage("{} 플레이어는 이미 외형 모음에 아이템 세트 |cffffffff|Hitemset:{}|h[{} {}]|h|r을 보유하고 있습니다.", nameLink, uint32(itemSetId), setName.c_str(), localeNames[locale]);
                handler->SetSentErrorMessage(true);
                return true;
            }
            ...
            if (target != handler->GetPlayer())
                handler->PSendSysMessage("아이템 세트 |cffffffff|Hitemset:{}|h[{} {}]|h|r을 {} 플레이어의 외형 모음에 추가했습니다.", uint32(itemSetId), setName.c_str(), localeNames[locale], nameLink);
        }

        // Notify target of new item in appearance collection
        if (target && !(target->GetPlayerSetting("mod-transmog", SETTING_HIDE_TRANSMOG).value))
            ChatHandler(target->GetSession()).PSendSysMessage("아이템 세트 |cffffffff|Hitemset:%d|h[{} {}]|h|r을 외형 모음에 추가했습니다.", uint32(itemSetId), setName.c_str(), localeNames[locale]);
    ...
    static bool HandleReloadTransmogConfig(ChatHandler* handler)
    {
        ...
        handler->SendSysMessage("형상변환 설정을 다시 불러왔습니다.");
        sTransmogrification->LoadCollections();
        handler->SendSysMessage("형상변환 외형 모음을 다시 불러왔습니다.");
        return true;
    }
};
