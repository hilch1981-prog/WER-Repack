    ...
    for (uint16 slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot)
    {
        ...
        if (player->GetSession())
            if (QuestLocale const* locale = sObjectMgr->GetQuestLocale(questId))
                ObjectMgr::GetLocaleString(locale->Title,
                    player->GetSession()->GetSessionDbLocaleIndex(), questTitle);
        ...
        for (QuestPOI const& poi : *poiVector)
        {
            ...
            result.push_back({ questId, questTitle, poi.Id, bestX, bestY, bestDist, areaX, areaY, areaRadius, poi.ObjectiveIndex });
        }
    }
...
void QuestRadar_AnnounceNearbyObjectives(Player* player, uint32 maxCount)
{
    ...
    if (objectives.empty())
    {
        if (player->GetSession()->GetSessionDbLocaleIndex() == LOCALE_koKR)
            handler.PSendSysMessage("|cffffcc00[QuestRadar]|r 이 지도에서 위치를 확인할 수 있는 퀘스트 목표가 없습니다.");
        else
            handler.PSendSysMessage("|cffffcc00[퀘스트 탐색]|r 이 지도에서 위치를 확인할 수 있는 퀘스트 목표가 없습니다.");
        return;
    }
    ...
    for (QuestRadarObjective const& obj : objectives)
    {
        ...
        if (player->GetSession()->GetSessionDbLocaleIndex() == LOCALE_koKR)
            handler.PSendSysMessage("|cffffcc00[QuestRadar]|r {} - {} 야드 (X: {:.1f}, Y: {:.1f})",
                obj.questTitle, uint32(obj.distance), obj.x, obj.y);
        else
            handler.PSendSysMessage("|cffffcc00[퀘스트 탐색]|r {} - {}미터 (X: {:.1f}, Y: {:.1f})",
                obj.questTitle, uint32(obj.distance), obj.x, obj.y);
        ++shown;
    }
}
...
std::vector<std::string> QuestRadar_BuildAddonSyncMessages(Player* player)
{
    ...
    for (QuestRadarObjective const& obj : objectives)
    {
        ...
        std::string title = QuestRadar_WireTitle(obj.questTitle);
