...
#include "QuestRadarRequestGate.h"
#include "Chat.h"
...
public:
    ...
    {
        if (player)
            requestGate_.Remove(player->GetGUID().GetRawValue());
    }
    ...
    void OnPlayerBeforeSendChatMessage(Player* player, uint32& type, uint32& lang, std::string& msg) override
    {
        ...
        if (!player || !player->GetSession() || !requestGate_.Accept(player->GetGUID().GetRawValue()))
            return;
...
private:
    QuestRadarRequestGate requestGate_;
};
...
public:
    ...
    static bool HandleQuestRadarCommand(ChatHandler* handler)
    {
        ...
        {
            if (handler->GetSession() && handler->GetSession()->GetSessionDbLocaleIndex() == LOCALE_koKR)
                handler->PSendSysMessage("|cffffcc00[QuestRadar]|r 모듈이 비활성화되어 있습니다 (QuestRadar.Enable = 0).");
            else
                handler->PSendSysMessage("|cffffcc00[퀘스트 탐색]|r 모듈이 꺼져 있습니다 (QuestRadar.Enable = 0).");
            return true;
        }
