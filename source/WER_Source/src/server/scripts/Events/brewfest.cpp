...
struct npc_dark_iron_attack_generator : public ScriptedAI
{
    ...
    {
        LOG_INFO("scripts.brewfest", "Dark Iron attack ended: map={} result=kegs_destroyed guzzlers={}", me->GetMapId(), guzzlerCounter);
    ...
    {
        LOG_INFO("scripts.brewfest", "Dark Iron attack ended: map={} result=kegs_defended guzzlers={}", me->GetMapId(), guzzlerCounter);
        if (Creature* herald = me->FindNearestCreature(NPC_DARK_IRON_HERALD, 100.0f))
        {
            char amount[500];
    ...
    void PrepareEvent()
    {
        ...
        LOG_INFO("scripts.brewfest", "Dark Iron attack started: map={}", me->GetMapId());
    }
...
struct npc_dark_iron_guzzler : public ScriptedAI
{
    ...
    {
        if (type != POINT_MOTION_TYPE || !targetGUID || id != targetGUID.GetCounter() || !me->IsAlive())
        {
            return;
        }
    ...
    void FindNextKeg()
    {
        ...
        for (uint8 i = 0; i < 3; ++i)
            ...
            {
                // Kegs are stationary and can despawn during the attack. Keep only their
                // GUID, not a persistent FollowMovementGenerator reference to the keg.
                float x, y, z;
                cr->GetClosePoint(x, y, z, me->GetCombatReach(), 1.0f, cr->GetAngle(me) - cr->GetOrientation());
                targetGUID = cr->GetGUID();
                me->GetMotionMaster()->MovePoint(targetGUID.GetCounter(), x, y, z);
                return;
            }
