...
public:
    ...
    */
    ...
    // WL crashfix: safety net called from object destructors. If an object is freed while still
    // tracked in _updatableObjectList (a deletion path that bypassed Map::RemoveFromMap), remove it
    // now -- prevents the map tick dereferencing a freed pointer -- and log the leak to "wowlegends".
    void RemoveDestroyedObjectFromUpdateList(WorldObject* obj);
