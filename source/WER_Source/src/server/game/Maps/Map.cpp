...
Map::~Map()
{
    ...
    // WL crashfix: objects can outlive the map (owned elsewhere, e.g. pets/summons). Clear their
    // owner-map pointers so a later destructor doesn't try to self-remove from a FREED map's list.
    for (WorldObject* obj : _updatableObjectList)
        if (UpdatableMapObject* u = dynamic_cast<UpdatableMapObject*>(obj))
        {
            u->SetUpdateState(UpdatableMapObject::UpdateState::NotUpdating);
            u->SetUpdateListOwnerMap(nullptr);
        }
...
void Map::AddToGrid(Creature* obj, Cell const& cell)
{
    ...
    if (obj->IsFarVisible())
    {
        ...
        // WL crashfix: remember the registered location (see WorldObject::VisReg)
        obj->VisReg().farVisMap   = this;
        obj->VisReg().farVisGridX = cell.GridX();
        obj->VisReg().farVisGridY = cell.GridY();
        obj->VisReg().farVisCellX = cell.CellX();
        obj->VisReg().farVisCellY = cell.CellY();
    }
...
/*
 * WL crash guard (SEH containment) for the map update tick. The playerbot fork has
 * residual use-after-free crashes on the MapUpdater worker threads: a freed creature
 * or bot left in / mutated during the map update is dereferenced in Creature/Unit/
 * Player::Update (e.g. UpdateMovementFlags -> GetFloorZ through a garbage vtable, or a
 * corrupt aura map in _UpdateSpells) -> ACCESS_VIOLATION that kills the worldserver.
 * The root cure is a large refactor; until then, contain the hardware exception at the
 * per-object boundary and LOG it loudly (channel "wowlegends"):
 *   - non-player objects: the faulting object is removed from the update list by pointer
 *     (no deref - it may be freed) so it cannot re-fault every tick.
 *   - players: after repeated faults the session is kicked (one bot sacrificed, not 500).
 * Containment, not a cure - every catch stays an upstream bug. MSVC-only (SEH);
 * other platforms run unguarded. Config: WowLegends.CrashGuard.Enable (default 1).
 */
namespace
{
    std::mutex g_wlMapGuardMutex;
    std::unordered_map<uint64, uint32> g_wlPlayerUpdateFaults;
    constexpr uint32 WL_PLAYER_FAULTS_BEFORE_KICK = 3;

    // A function using __try must not need C++ unwinding -> POD-only helpers.
    struct WLUpdateDiagnostic
    {
        uint64 guid = 0;
        uint32 entry = 0;
        uint32 type = 0;
    ...
    bool WLGuardedObjectUpdate(WorldObject* obj, uint32 diff, uint32* code, WLUpdateDiagnostic* diagnostic)
    {
        __try
        {
            // Capture inside the SEH boundary; the pointer can already be stale.
            diagnostic->guid = obj->GetGUID().GetRawValue();
            diagnostic->entry = obj->GetEntry();
            diagnostic->type = obj->GetTypeId();
            obj->Update(diff);
            return true;
        }
        __except (*code = GetExceptionCode(),
            diagnostic->instruction = reinterpret_cast<uintptr_t>(GetExceptionInformation()->ExceptionRecord->ExceptionAddress),
    ...
    bool WLGuardedPlayerUpdate(Player* player, uint32 diff, uint32* code)
    {
        __try
        {
            player->Update(diff);
    ...
    bool WLMapGuardEnabled()
    {
        static bool const enabled = sConfigMgr->GetOption<bool>("WowLegends.CrashGuard.Enable", true);
        return enabled;
    }

    void WLHandlePlayerUpdateFault(Player* player, uint32 code)
    {
        uint64 const rawGuid = player->GetGUID().GetRawValue();
        uint32 faults = 0;
        {
            std::lock_guard<std::mutex> lock(g_wlMapGuardMutex);
            faults = ++g_wlPlayerUpdateFaults[rawGuid];
        }
        // Log only the first fault and the kick threshold. KickPlayer is a no-op for socketless
        // playerbot sessions (it cannot remove them), so a persistently-faulting bot would otherwise
        // flood the logs every tick over a long soak. Further faults are contained silently.
        if (faults == 1)
            LOG_ERROR("wowlegends", "[crashguard] exception {:#x} in player Update for {} (guid {}) - tick skipped",
                code, player->GetName(), rawGuid);
        else if (faults == WL_PLAYER_FAULTS_BEFORE_KICK)
        {
            LOG_ERROR("wowlegends", "[crashguard] {} (guid {}) hit the fault limit (exception {:#x}) - kicking (no-op for bot sessions); further faults contained silently",
                player->GetName(), rawGuid, code);
            if (WorldSession* session = player->GetSession())
                session->KickPlayer("WL crash guard: repeated faults in player Update");
        }
    }
}
#endif
...
void Map::Update(const uint32 t_diff, const uint32 s_diff, bool  /*thread*/)
{
    ...
    for (m_mapRefIter = m_mapRefMgr.begin(); m_mapRefIter != m_mapRefMgr.end(); ++m_mapRefIter)
    {
        ...
        if (player && player->IsInWorld())
        {
            ...
            {
#if defined(_MSC_VER)
                if (WLMapGuardEnabled())
                {
                    uint32 code = 0;
                    if (!WLGuardedPlayerUpdate(player, s_diff, &code))
                        WLHandlePlayerUpdateFault(player, code);
                }
                else
...
void Map::UpdateNonPlayerObjects(uint32 const diff)
{
    ...
    if (_updatableObjectListRecheckTimer.Passed())
    {
        for (uint32 i = 0; i < _updatableObjectList.size();)
        {
            ...
            {
                uint32 code = 0;
                WLUpdateDiagnostic diagnostic;
                if (!WLGuardedObjectUpdate(obj, diff, &code, &diagnostic))
                {
                    LOG_ERROR("wowlegends", "[crashguard] exception {:#x} in non-player object Update at {:#x}; guid={} entry={} type={} map={} instance={} instruction={:#x}",
                        code, reinterpret_cast<uintptr_t>(obj), diagnostic.guid, diagnostic.entry, diagnostic.type, GetId(), GetInstanceId(), diagnostic.instruction);
                    // Update may already have removed itself before the exception.
                    // Never discard the unrelated object that replaced slot i.
                    if (i >= _updatableObjectList.size() || _updatableObjectList[i] != obj)
                        continue;
                    // obj may be freed: remove it WITHOUT dereferencing it. obj sits at slot i, so by
                    // the list invariant its offset == i. Mirror _RemoveObjectFromUpdateList's swap but
                    // deref only the SURVIVOR moved into slot i (a valid Updating object), never obj.
                    // obj's own state is left stale; if it was freed it is already gone, and if it
                    // survived, _RemoveObjectFromUpdateList's offset-validation guard makes a later
                    // removal safe.
                    WorldObject* survivor = _updatableObjectList.back();
                    if (survivor != obj)
                    {
                        _updatableObjectList[i] = survivor;
                        dynamic_cast<UpdatableMapObject*>(survivor)->SetMapUpdateListOffset(i);
                    }
                    _updatableObjectList.pop_back();
                    continue;   // re-process slot i (now the survivor, or the loop ends)
                }
            }
            else
                obj->Update(diff);
            ...
            // WL crashfix: obj may have DELETED ITSELF during Update (its destructor self-removes
            // from this list, swapping another element into slot i). Touching obj after Update is
            // only safe while slot i still holds the same pointer (pointer compare only, no deref;
            // the destructor net guarantees a freed object is no longer in the list).
            if (i >= _updatableObjectList.size() || _updatableObjectList[i] != obj)
                continue;   // slot i now holds a swapped-in element (or the list shrank) - re-process it
        ...
        for (uint32 i = 0; i < _updatableObjectList.size();)
        {
            ...
            if (WLMapGuardEnabled())
            {
                ...
                if (!WLGuardedObjectUpdate(obj, diff, &code, &diagnostic))
                {
                    ...
                    if (i >= _updatableObjectList.size() || _updatableObjectList[i] != obj)
                        continue;
                    // remove without dereferencing obj (may be freed); fix the survivor's offset only.
                    WorldObject* survivor = _updatableObjectList.back();
                    if (survivor != obj)
                    {
                        _updatableObjectList[i] = survivor;
...
void Map::AddObjectToPendingUpdateList(WorldObject* obj)
{
    ...
    mapUpdatableObject->SetUpdateListOwnerMap(this);   // WL crashfix: track which map's list holds us
}
...
void Map::_AddObjectToUpdateList(WorldObject* obj)
{
    ...
    mapUpdatableObject->SetUpdateListOwnerMap(this);   // WL crashfix (defensive; set at PendingAdd already)
...
void Map::_RemoveObjectFromUpdateList(WorldObject* obj)
{
    ...
    // WL crashfix: the map crash-guard removes a faulting object from _updatableObjectList by index
    // without resetting its state (it cannot safely deref a possibly-freed object). If such an object
    // SURVIVED the fault and is later removed normally, its stored offset is stale. Validate it points
    // at this object before touching the vector, else just clear the state (no out-of-bounds swap).
    std::size_t const offset = mapUpdatableObject->GetMapUpdateListOffset();
    if (offset >= _updatableObjectList.size() || _updatableObjectList[offset] != obj)
    {
        mapUpdatableObject->SetUpdateState(UpdatableMapObject::UpdateState::NotUpdating);
        mapUpdatableObject->SetUpdateListOwnerMap(nullptr);
        return;
    }

    if (obj != _updatableObjectList.back())
    {
        dynamic_cast<UpdatableMapObject*>(_updatableObjectList.back())->SetMapUpdateListOffset(offset);
        std::swap(_updatableObjectList[offset], _updatableObjectList.back());
    }

    _updatableObjectList.pop_back();
    mapUpdatableObject->SetUpdateState(UpdatableMapObject::UpdateState::NotUpdating);
    mapUpdatableObject->SetUpdateListOwnerMap(nullptr);   // WL crashfix
}
...
void Map::RemoveObjectFromMapUpdateList(WorldObject* obj)
{
    ...
    if (mapUpdatableObject->GetUpdateState() == UpdatableMapObject::UpdateState::PendingAdd)
    {
        ...
        // WL crashfix: also reset the state (was left as PendingAdd -> object could never be
        // re-added) and the owner-map pointer.
        mapUpdatableObject->SetUpdateState(UpdatableMapObject::UpdateState::NotUpdating);
        mapUpdatableObject->SetUpdateListOwnerMap(nullptr);
    }
...
// WL crashfix: invoked from Creature/GameObject/DynamicObject destructors. The normal removal
// (Map::RemoveFromMap) already pulls objects from _updatableObjectList before freeing them, but a
// deletion path that bypasses it would leave a DANGLING pointer in the list -> the next map tick
// calls obj->Update() on freed memory (the recurring use-after-free crash on the map worker thread).
// This is the one chokepoint every object passes through (destruction), so we remove it here as a
// last resort and log which object leaked so the offending path can be found/reported.
void Map::RemoveDestroyedObjectFromUpdateList(WorldObject* obj)
{
    if (!obj->CanBeAddedToMapUpdateList())
        return;
    ...
    UpdatableMapObject* mapUpdatableObject = dynamic_cast<UpdatableMapObject*>(obj);
    if (!mapUpdatableObject || mapUpdatableObject->GetUpdateState() == UpdatableMapObject::UpdateState::NotUpdating)
        return; // normal case: already removed cleanly, nothing to do

    LOG_ERROR("wowlegends", "[updatelist-leak] {} (entry {}) destroyed while still in map {} update-list (state {}); removed defensively to prevent a map-tick use-after-free.",
        obj->GetGUID().ToString(), obj->GetEntry(), GetId(), uint32(mapUpdatableObject->GetUpdateState()));
...
void Map::AddWorldObjectToFarVisibleMap(WorldObject* obj)
{
    ...
    // WL crashfix: remember the registered location (see WorldObject::VisReg)
    Cell curr_cell = obj->ToCreature() ? obj->ToCreature()->GetCurrentCell() : obj->ToGameObject()->GetCurrentCell();
    obj->VisReg().farVisMap   = this;
    obj->VisReg().farVisGridX = curr_cell.GridX();
    obj->VisReg().farVisGridY = curr_cell.GridY();
    obj->VisReg().farVisCellX = curr_cell.CellX();
    obj->VisReg().farVisCellY = curr_cell.CellY();
}

void Map::RemoveWorldObjectFromFarVisibleMap(WorldObject* obj)
{
    // WL crashfix: erase the REGISTERED location (stamped at insert) when we have one --
    // an erase computed from a stale current-cell silently misses the entry and leaves a
    // dangling pointer for the visibility walk (live UAF 2026-06-11 14:09). Fall back to
    // the current cell only for unstamped objects.
    WorldObject::VisibilityRegistration& reg = obj->VisReg();
    bool const stamped = (reg.farVisMap == this);

    if (Creature* creature = obj->ToCreature())
    {
        Cell curr_cell = creature->GetCurrentCell();
        uint32 const gx = stamped ? reg.farVisGridX : curr_cell.GridX();
        uint32 const gy = stamped ? reg.farVisGridY : curr_cell.GridY();
        if (MapGridType* grid = GetMapGrid(gx, gy))
            grid->RemoveFarVisibleObject(stamped ? reg.farVisCellX : curr_cell.CellX(),
                                         stamped ? reg.farVisCellY : curr_cell.CellY(), creature);
    }
    else if (GameObject* go = obj->ToGameObject())
    {
        Cell curr_cell = go->GetCurrentCell();
        ...
        if (MapGridType* grid = GetMapGrid(gx, gy))
            grid->RemoveFarVisibleObject(stamped ? reg.farVisCellX : curr_cell.CellX(),
                                         stamped ? reg.farVisCellY : curr_cell.CellY(), go);
    }

    if (stamped)
        reg.farVisMap = nullptr;
}
...
void Map::AddWorldObjectToZoneWideVisibleMap(uint32 zoneId, WorldObject* obj)
{
    ...
    // WL crashfix: remember the registered zone (see WorldObject::VisReg)
    obj->VisReg().zoneWideMap    = this;
    obj->VisReg().zoneWideZoneId = zoneId;
}

void Map::RemoveWorldObjectFromZoneWideVisibleMap(uint32 zoneId, WorldObject* obj)
{
    // WL crashfix: the REGISTERED zone (stamped at insert) wins over the caller's zoneId --
    // a stale caller zone makes the erase silently miss, leaving a dangling pointer.
    WorldObject::VisibilityRegistration& reg = obj->VisReg();
    uint32 const useZone = (reg.zoneWideMap == this) ? reg.zoneWideZoneId : zoneId;

    ZoneWideVisibleWorldObjectsMap::iterator itr = _zoneWideVisibleWorldObjectsMap.find(useZone);
    if (itr != _zoneWideVisibleWorldObjectsMap.end())
        itr->second.erase(obj);

    if (reg.zoneWideMap == this)
    {
        reg.zoneWideMap    = nullptr;
        reg.zoneWideZoneId = 0;
    }
}
