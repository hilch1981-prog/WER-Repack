...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket& recvData)
{
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
    {
        ...
        if (quest->IsAutoAccept() && _player->CanAddQuest(quest, true))
        {
            ...
            // WL: auto-accepted quests (quest_template_addon.SpecialFlags & 4 -
            // the whole starter-zone experience) never produce
            // CMSG_QUESTGIVER_ACCEPT_QUEST, so the playerbots master-packet
            // mirror (bot quest sync) is deaf to them. Synthesize the accept
            // so grouped bots follow auto-accept quests too.
            WorldPacket wlAccept(CMSG_QUESTGIVER_ACCEPT_QUEST, 8 + 4 + 4);
            wlAccept << guid << questId << uint32(0);
            sScriptMgr->OnPacketReceived(this, wlAccept);
#endif
        }
