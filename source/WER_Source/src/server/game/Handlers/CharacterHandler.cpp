...
void WorldSession::HandlePlayerLoginFromDB(LoginQueryHolder const& holder)
{
    ...
    // Refresh the 3.3.5 client name cache when returning from character selection.
    SendNameQueryOpcode(pCurrChar->GetGUID());
    if (Group* group = pCurrChar->GetGroup())
        for (Group::MemberSlot const& member : group->GetMemberSlots())
            if (member.guid != pCurrChar->GetGUID())
                SendNameQueryOpcode(member.guid);
