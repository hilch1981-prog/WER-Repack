...
// mod-wowlegends (Paths of Legends): the Iron Oath seals the mailbox.
// Defined in modules/mod-wowlegends/src/wowlegends_paths.cpp - this is the
// ONE choke point every mail opcode passes (list/take/delete/return/send).
bool WlPathsMailboxSealed(Player* player);

bool WorldSession::CanOpenMailBox(ObjectGuid guid)
{
    if (WlPathsMailboxSealed(_player))
        return false;
