...
Creature::~Creature()
{
    ...
    // Use the stored owner-map (set when the list took us, cleared when it released us) -- NOT
    // m_currMap/FindMap(), which is already nulled by ResetMap() on most deletion paths, hiding
    // the leak from the destructor (proven 2026-06-10: UAF recurred with the FindMap() version).
    if (Map* listOwner = GetUpdateListOwnerMap())
        listOwner->RemoveDestroyedObjectFromUpdateList(this);

    // WL crashfix: same idea for a TRANSPORT's passenger set (it holds raw WorldObject*).
    // Cleanup paths pair RemovePassenger + SetTransport(nullptr), so a non-null transport here
    // means we are STILL in its set -- remove ourselves or its update tick relocates freed
    // memory (live UAF 2026-06-11 10:02, Transport.cpp UpdatePassengerPositions).
    if (Transport* transport = GetTransport())
    {
        LOG_ERROR("wowlegends", "[transport-leak] {} (entry {}) destroyed while still in a transport's passenger set - removed defensively.",
            GetGUID().ToString(), GetEntry());
        transport->RemovePassenger(this, true);
        SetTransport(nullptr);
    }

    // WL crashfix: purge any still-registered visibility-container entries (far-visible /
    // zone-wide hold raw pointers; a missed removal leaves a dangling pointer the visibility
    // walk dereferences - live UAF 2026-06-11 14:09). Registration is stamped on insert, so
    // this removes from exactly where we were inserted.
    if (Map* visMap = VisReg().farVisMap)
    {
        LOG_ERROR("wowlegends", "[farvis-leak] {} (entry {}) destroyed while still in the far-visible container - removed defensively.",
            GetGUID().ToString(), GetEntry());
        visMap->RemoveWorldObjectFromFarVisibleMap(this);
    }
    if (Map* zoneMap = VisReg().zoneWideMap)
    {
        LOG_ERROR("wowlegends", "[zonevis-leak] {} (entry {}) destroyed while still in the zone-wide-visible container (zone {}) - removed defensively.",
