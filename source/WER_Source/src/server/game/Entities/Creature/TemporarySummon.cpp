...
{
    if (m_deathState == DeathState::Dead)   // WL: despawn an already-dead summon BEFORE Creature::Update (its spline->UpdateMovementFlags->GetFloorZ path UAF-crashes on a freed Raging Spirit)
    {
        UnSummon();
        return;
    }
    ...
    if (m_deathState == DeathState::Dead)   // may have died during this tick
    {
        UnSummon();
        return;
    }
