...
bool Pet::LoadPetFromDB(Player* owner, uint32 petEntry, uint32 petnumber, bool current, uint32 healthPct /*= 0*/, bool fullMana /*= false*/)
{
    ...
    // WasRenamed is derived from UNIT_CAN_BE_RENAMED, which is absent on summons.
    // Only hunters use it to distinguish a user-chosen name (HandlePetRename).
    if (sWorld->GetDefaultDbcLocale() == LOCALE_koKR)
    {
        if (getPetType() == HUNTER_PET && !petInfo->WasRenamed)
        {
            if (CreatureFamilyEntry const* family = sCreatureFamilyStore.LookupEntry(GetCreatureTemplate()->family))
                if (family->Name[LOCALE_koKR] && family->Name[LOCALE_koKR][0])
                    SetName(family->Name[LOCALE_koKR]);
        }
        else if (getPetType() == SUMMON_PET &&
            petInfo->Name.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ '-") == std::string::npos)
        {
            std::string localizedName = sObjectMgr->GeneratePetNameLocale(GetEntry(), LOCALE_koKR);
            // Some summons (e.g. Water Elemental) have no generated-name pool.
            if (localizedName.empty() ||
                localizedName.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ '-") == std::string::npos)
            {
                if (CreatureLocale const* locale = sObjectMgr->GetCreatureLocale(GetEntry()))
                    if (locale->Name.size() > LOCALE_koKR && !locale->Name[LOCALE_koKR].empty())
                        localizedName = locale->Name[LOCALE_koKR];
            }
            if (!localizedName.empty())
