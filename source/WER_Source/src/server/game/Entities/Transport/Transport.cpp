            ...
            case TYPEID_UNIT:
                {
                    ...
                    // WL crashfix: mirror the TYPEID_PLAYER branch below -- do NOT relocate a creature
                    // that has been removed from the world. A stale/dangling creature left in the
                    // transport's passenger set was being relocated here, UAF-crashing on the map
                    // worker thread (CreatureRelocation -> UpdatePositionData -> UpdateMovementFlags
                    // -> GetFloorZ -> virtual GetCollisionHeight() through a freed vtable).
