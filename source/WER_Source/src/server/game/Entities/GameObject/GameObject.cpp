...
{
    // WL crashfix: ensure we're not left as a dangling pointer in a map's _updatableObjectList.
    // Uses the stored owner-map, not m_currMap (already nulled by ResetMap on most delete paths).
    if (Map* listOwner = GetUpdateListOwnerMap())
        listOwner->RemoveDestroyedObjectFromUpdateList(this);

    // WL crashfix: same for a transport's passenger set (raw pointers; see Creature dtor).
    if (Transport* transport = GetTransport())
    {
        LOG_ERROR("wowlegends", "[transport-leak] GO {} (entry {}) destroyed while still in a transport's passenger set - removed defensively.",
            GetGUID().ToString(), GetEntry());
        transport->RemovePassenger(this, true);
        SetTransport(nullptr);
    }

    // WL crashfix: purge still-registered visibility-container entries (see Creature dtor).
    if (Map* visMap = VisReg().farVisMap)
    {
        LOG_ERROR("wowlegends", "[farvis-leak] GO {} (entry {}) destroyed while still in the far-visible container - removed defensively.",
            GetGUID().ToString(), GetEntry());
        visMap->RemoveWorldObjectFromFarVisibleMap(this);
    }
    if (Map* zoneMap = VisReg().zoneWideMap)
    {
        LOG_ERROR("wowlegends", "[zonevis-leak] GO {} (entry {}) destroyed while still in the zone-wide-visible container (zone {}) - removed defensively.",
            GetGUID().ToString(), GetEntry(), VisReg().zoneWideZoneId);
        zoneMap->RemoveWorldObjectFromZoneWideVisibleMap(VisReg().zoneWideZoneId, this);
    }

    delete m_AI;

    // WL crashfix: a GameObjectModel freed while still registered in a map's dynamic
    // collision tree leaves a dangling pointer that the BIH rebalance dereferences
    // (live crash in BIH::subdivide via Creature::GetFloorZ -> DynamicMapTree::getHeight).
    // RemoveFromWorld only pulls the model while IsInWorld(); a GO destroyed outside that
    // path leaks it. Net it here using the stored owner map (NOT GetMap(), which asserts
    // on a nulled m_currMap during teardown), guarded so it is a no-op on clean paths.
    if (m_model)
    {
        if (Map* modelMap = GetUpdateListOwnerMap())
        {
            if (modelMap->ContainsGameObjectModel(*m_model))
            {
                LOG_ERROR("wowlegends", "[gomodel-leak] GO {} (entry {}) destroyed while still in the dynamic collision tree - removed defensively.",
                    GetGUID().ToString(), GetEntry());
                modelMap->RemoveGameObjectModel(*m_model);
            }
        }
    }
