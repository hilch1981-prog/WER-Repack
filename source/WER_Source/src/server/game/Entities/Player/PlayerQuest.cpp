        ...
        if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
        {
            ...
            else
            {
                ...
                if (quest->IsAutoAccept() && CanAddQuest(quest, true) && CanTakeQuest(quest, true))
                {
                    ...
                    // WL: auto-accepted quests never produce CMSG_QUESTGIVER_
                    // ACCEPT_QUEST, so the playerbots master-packet mirror
                    // (bot quest sync) is deaf to them. Synthesize the accept
                    ...
                    sScriptMgr->OnPacketReceived(GetSession(), wlAccept);
#endif
                }
