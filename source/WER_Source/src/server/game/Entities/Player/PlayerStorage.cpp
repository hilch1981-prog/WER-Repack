...
{
    // WL crashfix: a null item template (Item::GetTemplate() can return nullptr for an
    // inventory item whose entry is missing from item_template) must not be dereferenced.
    // Treat it as not totem-compatible instead of crashing the map-update worker thread.
    if (!pProto)
        return false;
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timeDiff)
{
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (Item* item = _LoadItem(trans, zoneId, timeDiff, fields))
            {
                ...
                else
                {
                    ...
                    if (GetSession()->IsBot() && bagGuid == 0 && slot < EQUIPMENT_SLOT_END &&
                        (err == EQUIP_ERR_NO_REQUIRED_PROFICIENCY || err == EQUIP_ERR_CANT_EQUIP_SKILL))
                        trans->Append("INSERT INTO wowlegends_bot_equipment_repair (guid,slot,source_item) VALUES ({},{},{}) ON DUPLICATE KEY UPDATE completed=0",
                            GetGUID().GetCounter(), slot, item->GetGUID().GetCounter());
                }
            }
        } while (result->NextRow());
        ...
        while (!problematicItems.empty())
        {
            ...
            MailDraft draft(subject, GetSession()->GetSessionDbLocaleIndex() == LOCALE_koKR
                ? "장비를 착용하거나 소지품에 보관할 수 없어 우편으로 보내 드립니다. 장비의 사용 조건과 가방 공간을 확인해 주세요."
