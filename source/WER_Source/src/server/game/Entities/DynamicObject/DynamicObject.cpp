...
DynamicObject::~DynamicObject()
{
    ...
    {
        LOG_ERROR("wowlegends", "[transport-leak] DynObject {} destroyed while still in a transport's passenger set - removed defensively.",
            GetGUID().ToString());
        transport->RemovePassenger(this, true);
        SetTransport(nullptr);
    }

    // WL crashfix: purge a still-registered zone-wide visibility entry (see Creature dtor).
    // (Far-visible vectors are Creature/GameObject-typed; dynobjects can't be in them.)
    if (Map* zoneMap = VisReg().zoneWideMap)
    {
        LOG_ERROR("wowlegends", "[zonevis-leak] DynObject {} destroyed while still in the zone-wide-visible container (zone {}) - removed defensively.",
            GetGUID().ToString(), VisReg().zoneWideZoneId);
