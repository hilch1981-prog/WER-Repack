...
void WorldObject::UpdatePositionData()
{
    ...
    Map* map = FindMap();          // WL: non-asserting; bail if object has no valid map (freed/removed summon) -> avoids GetMap() ASSERT-elided UAF deref
    if (!map)
        return;

    PositionFullTerrainStatus data;
    map->GetFullTerrainStatusForPosition(GetPhaseMask(), GetPositionX(), GetPositionY(), GetPositionZ(), GetCollisionHeight(), data);
...
float WorldObject::GetFloorZ() const
{
    ...
    Map* map = FindMap();          // WL: non-asserting; null when object has no valid map (freed/removed summon) -> avoids GetMap() ASSERT-elided UAF deref
    if (!map)
        return _floorZ;

    return std::max<float>(_floorZ, map->GetGameObjectFloor(GetPhaseMask(), GetPositionX(), GetPositionY(), GetPositionZ() + std::max(GetCollisionHeight(), Z_OFFSET_FIND_HEIGHT)));
}
