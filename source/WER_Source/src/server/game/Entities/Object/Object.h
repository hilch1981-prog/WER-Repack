...
protected:
    UpdatableMapObject() : _mapUpdateListOffset(0), _mapUpdateState(NotUpdating), _updateListOwnerMap(nullptr) { }

    // WL crashfix: the map whose update list currently tracks this object (set on PendingAdd /
    // Updating, cleared on removal and on map teardown). Lets the destructor self-remove from the
    // RIGHT list even after ResetMap() nulled m_currMap -- m_currMap is NOT reliable at destroy time.
    Map* GetUpdateListOwnerMap() const { return _updateListOwnerMap; }

private:
    void SetUpdateListOwnerMap(Map* map) { _updateListOwnerMap = map; }
...
public:
    ...
    // WL crashfix: records WHERE this object is registered in the map's far-visible /
    // zone-wide visibility containers (raw-pointer containers). Stamped on insert,
    // cleared on erase. Removal then always erases the REGISTERED location (an erase
    // computed from a stale current-cell/zone silently misses, leaving a dangling
    // pointer the visibility walk dereferences - live UAF 2026-06-11), and the
    // destructor can purge any entry a deletion path missed.
    struct VisibilityRegistration
    {
        Map*   farVisMap = nullptr;
        uint32 farVisGridX = 0;
        uint32 farVisGridY = 0;
        uint32 farVisCellX = 0;
        uint32 farVisCellY = 0;
        Map*   zoneWideMap = nullptr;
        uint32 zoneWideZoneId = 0;
    };
    VisibilityRegistration& VisReg() { return _visReg; }
...
private:
    ...
    VisibilityRegistration _visReg;                    // WL crashfix (see VisReg())
