...
void Guild::HandleMemberDepositMoney(WorldSession* session, uint32 amount)
{
    ...
    // mod-wowlegends: a script zeroing the amount is a veto (the client
    // never sends 0 - see HandleGuildBankDepositMoney); without this a
    // zeroed deposit would still write a 0-copper bank log + broadcast
    if (!amount)
        return;
...
bool Guild::HandleMemberWithdrawMoney(WorldSession* session, uint32 amount, bool repair)
{
    ...
    // mod-wowlegends: a script zeroing the amount is a veto. No legit
    // caller passes 0 (repair costs are floored to 1), and without this a
    // zeroed guild-funded REPAIR would still repair with NOBODY paying -
    // Player::DurabilityRepair only checks the bool
    if (!amount)
        return false;
