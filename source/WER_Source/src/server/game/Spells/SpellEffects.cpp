...
void Spell::EffectSummonPet(SpellEffIndex effIndex)
{
    ...
    // Socketless bot sessions may keep enUS even on the Korean realm.
    LocaleConstant const petLocale = sWorld->GetDefaultDbcLocale() == LOCALE_koKR ?
        LOCALE_koKR : owner->GetSession()->GetSessionDbcLocale();
    std::string new_name = sObjectMgr->GeneratePetNameLocale(petentry, petLocale);
    if (!new_name.empty())
