...
/*
 * WL crash guard (SEH containment): the playerbot AI ticks inside the AfterUpdate hook
 * and its engine has known use-after-free bugs (e.g. a bot's AiObjectContext freed
 * mid-tick -> ACCESS_VIOLATION that kills the whole worldserver, observed live with
 * full stacks). Until that is fixed upstream, catch the hardware exception at the
 * per-player boundary: skip the faulting tick, LOG it loudly, and after repeated
 * faults kick that one session - one bot sacrificed instead of 500 players dropped.
 * Pure containment, not a cure: every catch stays an upstream bug, and the log lines
 * (channel "wowlegends") are the evidence. MSVC-only (SEH); other platforms unguarded.
 * Config: WowLegends.CrashGuard.Enable (default 1, read once at startup).
 */
namespace
{
    // A function using __try must not require C++ unwinding -> POD-only helper.
    bool GuardedAfterUpdateCall(PlayerScript* script, Player* player, uint32 p_time, uint32* code)
    {
        __try
        {
            script->OnPlayerAfterUpdate(player, p_time);
            return true;
        }
    ...
    std::mutex g_crashGuardMutex;
    std::unordered_map<uint64, uint32> g_crashGuardFaults;   // raw guid -> consecutive faults
    constexpr uint32 CRASH_GUARD_FAULTS_BEFORE_KICK = 3;
}
#endif
...
{
#if defined(_MSC_VER)
    // WL crash guard - see the block comment at the top of this file.
    static bool const guardEnabled = sConfigMgr->GetOption<bool>("WowLegends.CrashGuard.Enable", true);
    if (guardEnabled)
    {
        if (ScriptRegistry<PlayerScript>::EnabledHooks[PLAYERHOOK_ON_AFTER_UPDATE].empty())
            return;

        for (auto const& script : ScriptRegistry<PlayerScript>::EnabledHooks[PLAYERHOOK_ON_AFTER_UPDATE])
        {
            uint32 code = 0;
            if (GuardedAfterUpdateCall(script, player, p_time, &code))
                continue;

            uint64 const rawGuid = player->GetGUID().GetRawValue();
            uint32 faults = 0;
            {
                std::lock_guard<std::mutex> lock(g_crashGuardMutex);
                faults = ++g_crashGuardFaults[rawGuid];
            }
            LOG_ERROR("wowlegends", "[crashguard] exception {:#x} in player-update script for {} (guid {}) - tick skipped, fault {}/{}",
                code, player->GetName(), rawGuid, faults, CRASH_GUARD_FAULTS_BEFORE_KICK);

            if (faults >= CRASH_GUARD_FAULTS_BEFORE_KICK)
            {
                LOG_ERROR("wowlegends", "[crashguard] {} exceeded the fault limit - kicking the session to contain the corruption", player->GetName());
                {
                    std::lock_guard<std::mutex> lock(g_crashGuardMutex);
                    g_crashGuardFaults.erase(rawGuid);
                }
                if (WorldSession* session = player->GetSession())
                    session->KickPlayer("WL crash guard: repeated faults in the player-update script");
            }
        }
        return;
    }
